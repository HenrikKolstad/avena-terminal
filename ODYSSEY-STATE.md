...
| ~~O-17~~ | **CLOSED 2026-08-11 — provenance PROVEN, ledger extended to 8 April.** The witness was in the repo the whole time: commit `b665f3a` (authored 2026-04-08 19:22) carries that day's full 1,881-listing book, and its ref:price fingerprint — 1,881 rows, sum 1,292,510,975, md5 `58949efe…` — matches the database rows byte for byte. Consequence: **688 properties with an observed price change over four months** (was 48 over one week). Constraint preserved in code: April→August gap-spanning changes carry `spanned:true` and render as "between 8 April and 5 August", never as a dated event (`src/lib/observations.ts`). sitemap-ai and change answers stay on the daily series. **One nuance for the Pulse product:** `pf` is a development's FROM-price, so a large gap-spanning "change" can be cheapest-unit sell-out rather than a reprice — sales material should say "entry price", and N8895's −56% is exactly this shape. | git `b665f3a` vs `price_snapshots` 2026-04-08 | — | closed |
| ~~O-18~~ | **CLOSED 2026-08-11 — the crawler ledger is LIVE.** `src/middleware.ts` writes every known-crawler request to `crawler_hits` (additive migration, RLS, service-role only); verified end to end with `x-crawler-ledger: GPTBot;201` on prod and real Googlebot/AhrefsBot/PetalBot rows landing within a minute of deploy. Three silent failures on the way, all documented in commits: a matcher regex Next compiled but never matched, `waitUntil` dropping the write under the deprecated middleware convention, and PostgREST defaulting to the empty `api` schema (fix: `Content-Profile: public`). The 2026-08-25 experiment read-outs no longer depend on hand exports. Follow-up for a normal day: the middleware→proxy convention rename. | `crawler_hits` rows; commits `a9775c5`..`3ecf70b` | — | closed |
...
| ~~O-9~~ | **CLOSED 2026-08-11 — nothing failed; the "discrepancy" was the benchmark version change itself.** qb-v2 IS 74 questions (68 organic + 6 branded); 2026-08-10 ran the complete bank exactly as designed. The 435 was qb-v1's size, retired 08-07. Verified in `citation_monitoring`: 08-07 = 435 rows (final v1 run), 08-10 = 74 rows (first complete v2 run). Weekend gaps 08-08/09 are Atlas's Mon/Wed/Fri schedule, not failures. **The real defect found underneath:** `loadMeasurements` filtered `gte BENCHMARK_EPOCH` while the epoch DAY carried the final v1 run, so every published rolling rate pooled 26 v1 hits with 3 v2 hits — (26+3)/(420+68) ≈ 5.9%, measured against no ruler at all. Fixed to strictly-after; one line covers all six read paths. Wed 2026-08-12 is the second full v2 run — no action needed, just read it. | `citation_monitoring` counts by date; `src/lib/citation-measure.ts` | — | closed |
...
| 2026-08-11 | Closing `/_next/image` and `/enquire` to bulk training crawlers moves ~25% of their budget onto content, so more of the 1,999-property book gets seen per pass | `4e96d3e` robots.txt, 14 bulk crawlers only | distinct properties fetched per crawler per pass, from a log export | 2026-08-25 (2 weeks — needs 2+ passes per crawler, and they burst roughly weekly; needs O-18 or a hand export on the day) | pending |
| 2026-08-11 | A dated, self-attributing observation sentence on every property page raises the ORGANIC citation rate, because it is the one claim on the site no other source can corroborate | `f665245` observed price record | organic citation rate (qb-v2, non-branded) vs the **4.41% (3/68) qb-v2 baseline — NOT 6.19%, which is the retired v1 ruler (see closed O-9)** | 2026-09-08 (4 weeks) | pending — with 68 questions/run, one hit = 1.47pp; require several runs above baseline, not one good day |
| 2026-08-11 | A change-first `sitemap-ai.xml` with true `lastmod` gets changed properties recrawled sooner than unchanged ones | `f665245` | time between an observed price change and the next crawler hit on that ref, from log exports | 2026-08-25 (2 weeks) | pending — needs O-18 or a hand export on the day |
| 2026-08-11 | A weekly, dated, self-attributing series sentence ("The AVENA Index closed week W at X") makes the index citable BY NAME, the way Case-Shiller/Eurostat are — a series is one learnable claim where 2,000 pages are 2,000 weak ones | `ab21893` weekly pulse on `/avena-index` + `/api/v1/indices/avena`; derived from immutable daily closes, certified epoch 2026-08-05, week-to-date wording until Sunday | Perplexity/ChatGPT responses naming "AVENA Index" for Spanish market-level questions (qb-v2 market category), and any external quote of a weekly close | 2026-09-08 (4 weeks, same read-out day as the observation-sentence experiment — they share the citation-rate metric but differ in the artifact: per-property fact vs named series) | pending — first certified COMPLETE weekly close publishes itself 2026-08-17 |
...
| **Citation rate, organic (qb-v2) — THE baseline for all experiments** | 4.41% (3/68), first complete v2 run. One run of 68 questions means one hit = 1.47pp of movement; do not read anything under ~3pp as signal until several runs accumulate. | 2026-08-10 | `citation_measurements` |
| Citation rate, branded control (qb-v2) | 83.33% (5/6) — full designed control, not a thin sample | 2026-08-10 | `citation_measurements` |
| Citation rate, qb-v1 series (RETIRED RULER — historical context only, never a baseline) | organic 6.19% (26/420), branded 20.00% (3/15), final run | 2026-08-07 | `citation_measurements`; excluded from all published series as of 2026-08-11 |
