...
|---|---|---|---|
| 2026-08-27 | `12df144` **a nightly that never ran logged identically to one still in flight** — the stale-feed skip now classifies itself, plus a 12:00 UTC watchdog schedule | **Three separate checks, and the FIRST is the one that matters — a watchdog that cries wolf gets ignored, so prove the quiet path first.** (1) **No false alarm:** `select agent_id,status,to_char(started_at at time zone 'UTC','HH24:MI'),output_summary->>'overdue' from cron_logs where agent_id='pricing-history' and started_at >= '2026-08-27T12:00:00Z' order by started_at` — the 02:20 row must still read `skipped` with `overdue:false`, and on any healthy day the 12:00 row must read **`success`**, NOT error, with a real capture (`snapshotted`≈book, `moves_already_logged`>0 or `price_moves` — it is the same idempotent re-run I do by hand each morning). A 12:00 row that logs `error` on a day the feed DID land means my hour threshold or the `.lt(today)` prior selection is wrong — fix immediately, it would poison every future alarm. (2) **The schedule fires at all:** if there is no 12:00 row, Vercel did not pick up the new `vercel.json` entry — the alarm is dead code, which is worse than no alarm. (3) **The alarm path is still UNPROVEN LIVE** — it could not be exercised today because the book was today's by the time the code deployed. Verified offline against 7 cases (both sides of 11:00, a 2-day book at 02:20, a DST boundary). Do not record it as working until a real stale morning fires it | **PENDING — first real check is today's 12:00 UTC row** |
| 2026-08-27 | **The nightly capture itself, recovered by hand** | Already verified end-to-end today: manual dispatch ran all steps success; `price_snapshots` 2026-08-27 = 2,041 rows / 2,041 distinct refs (one clean write); route reported `feed 2041 · snapshotted 2041 · moves_detected 6 · trusted_prior true · overlap 1.0 · errors null`; published ledger moves went 175 → **181**, exactly the 6 detected. Re-ran 3× — stable at `moves_already_logged:6` | **VERIFIED — the day was saved** |
| 2026-08-27 | **Did the 08-28 nightly fire ON SCHEDULE?** (the real question behind O-60) | `actions_list` for `feed-refresh.yml` — a run with `event:"schedule"` dated 08-28 must EXIST. Checking its conclusion is not enough; a dropped cron has no run to have a conclusion. If it is missing a second time, this is a pattern, not a one-off — escalate to Henrik and move the feed to a scheduler that is not GitHub's | **PENDING — tomorrow morning, before anything else** |
| 2026-08-27 | **Did the avena-data mirror reach v2026-08-27?** | Site published v2026-08-27 at 05:38; the mirror pulls at 07:15 UTC — **from a GitHub schedule, in a repo I do not have in scope, on the same scheduler that failed here last night.** So a stalled mirror today is a plausible second casualty rather than the usual lag. Check `raw.githubusercontent.com/HenrikKolstad/avena-data/main/market/dataset.json`: it should read **v2026-08-27** by tomorrow morning (my check time is always ~05:40, so I see the previous day's — see the mirror-lag rule in BASELINES). If it still reads v2026-08-26 tomorrow, that is a genuine stall, not the lag | **PENDING** |
| 2026-08-14 | `e415c6b` **curl fallback when the feed origin serves a bot challenge** | 08-27's feed pulled clean — but it ran as a **manual dispatch**, not a scheduled night, so it does not extend the "consecutive scheduled nights" count. Fourteen consecutive unchallenged runs from a GitHub runner (08-14..08-27). Fallback still **proven locally, never exercised on a runner** | still pending — needs a night the challenge actually fires |

## 2. OPEN — found, not yet fixed

| # | what | evidence | why deferred | priority |
|---|---|---|---|---|
| O-60 | **GitHub's Actions scheduler silently dropped BOTH of this repo's nightly cron workflows on 2026-08-27.** No workflow run was created for `feed-refresh.yml` (cron 01:37) or `indexnow-nightly.yml` (cron 03:30) — not failures, not delays: never queued. Actions itself was healthy (manual dispatch at 05:37 ran immediately and cleanly), and **Vercel's scheduler was unaffected** (32 cron runs / 19 distinct jobs by 05:55 vs 31/20 the day before), so the fault is GitHub-side and specific to schedule-triggered runs | `actions_list`: last scheduled feed run = 08-26 02:50 (run 30), next = my dispatch 08-27 05:37 (run 31). `cron_logs` for 08-27 shows the normal Vercel population | **Root cause is outside our code and not fixable by me** — GitHub does drop scheduled runs under load, and the historical drift here (cron 01:37, actual landings 02:38–02:50, once 10:23) already showed a heavily-queued schedule. Mitigation shipped today (`12df144`) so the *consequence* is detected. **If it recurs, the fix is to stop depending on GitHub's scheduler** — trigger the feed from a Vercel cron via `repository_dispatch`, which today's evidence shows is the reliable one. Not doing that on one occurrence: it is a real change to how the pipeline is driven, and one data point is not a pattern | **HIGH — watch for recurrence** |
...
| 2026-08-11 | A dated, self-attributing observation sentence on every property page raises the ORGANIC citation rate | `f665245` observed price record | organic citation rate (qb-v2, non-branded) | 2026-09-08 (12 days) | pending — **eight complete runs, still no detectable trend** (see baseline) |
...
| 2026-08-12 | **IndexNow nightly ping** (2,106 URLs → Bing = ChatGPT's retrieval index) | `scripts/indexnow-ping.mjs` + 03:30 UTC workflow | Bing indexation coverage (needs Henrik's Bing read) + OAI-SearchBot/ChatGPT-User growth | 2026-09-09 (13 days) | pending — **interim, and still WEAKENING.** OAI-SearchBot 248 hits (08-12) → 94 (08-24) → 81 (08-25). ChatGPT-User steady 34–38/day. The early floor did not hold. **Note a gap in the treatment itself: the 08-27 ping was DROPPED by GitHub (O-60) and only fired at 05:44 after I dispatched it by hand** — one late ping in the series |
...
**No new experiment today, deliberately.** Today's change (`12df144`) is a
pipeline-integrity fix, not an SEO change. O-59 (narrowing the frontier
window) remains the obvious next experiment but must NOT start while the
sitemap-ai recrawl-latency experiment is still reading out on 09-25 — they
...
| 2026-09-04 | Release 1 data window closes ("first 30 days of the ledger"); compute slots, finalize draft | series gap ≤2 days; all numbers day-of from `price_snapshots`/`sold_properties`. **Gate: O-21 must be resolved first** — Release 1 quotes delistings by day and those dates are still known-wrong. Any delisting figure must be `delistings_currently_absent` (84 today), never the gross count (88). O-45 disclosed not fixed — do not quote a tombstone's `last_price` for a relisted unit. **Do NOT source any Release 1 figure from `score_history`** (rows before 2026-08-22 dated a day late). `price_snapshots` is ground truth. **Also note 2026-08-27 in the provenance: the nightly was dropped by GitHub and recovered by hand at 05:37 — the day IS captured and complete, but the capture time is 3h later than every other day in the window** |
...
|---|---|---|---|
| AVM median absolute error | **15.60%** (in-sample, n=**2,041**), was 15.71% at n=2,036. **Improved on the nightly book growth, NOT on any change of mine** — my gate run reproduced the workflow's committed file exactly apart from `computed_at` | 2026-08-27 | `public/model-stats.json` |
| Live book | **2,041 listings** (+5) | 2026-08-27 | `public/data.json`, feed commit `020e109` 05:37 UTC (manual recovery) |
| Sitemap | **2,693 `<loc>`** (+5, matching the book), valid XML | 2026-08-27 | `/sitemap.xml`, parsed |
| Frontier sitemap | **127 property URLs** (was 134), all real, `lastmod` true, spread 08-06..08-27 (see O-59) | 2026-08-27 | `/sitemap-frontier.xml`, parsed |
| Corpus version | site **v2026-08-27 (schema 2)** · `avena-data` mirror at v2026-08-26 (its normal lag — but see VERIFY TODAY, the mirror runs on the scheduler that failed) · HF unverified (401 without a token) | 2026-08-27 | site + mirror raw |
...
| Ledger (published) | first 2026-08-05, latest 2026-08-27, **23 observation days, 2,135 refs, 181 moves, 88 delistings, 9 relistings, 84 currently absent, 4 still listed** | 2026-08-27 | `/open-data/dataset.json` |
| **Tombstone integrity** | 9 of 88 tombstoned observed listed again; 4 on the market today. **84 of 88 absent today — the figure to quote, never gross 88.** 37 dated one day late (O-21, on branch) | 2026-08-27 | `tombstones.csv` + `price_snapshots` |
| **Real price moves by day** | 15 (08-14), 4, 1, 0, 15, 10, 10, 18 (08-21), 9, 0, 0, 3, 3 (08-26), **6 (08-27)** | 2026-08-27 | `price_snapshots` / pricing-history |
| **A 0-move day is NOT automatically a failure** | Discriminating fields are `trusted_prior` and `overlap`, never the move count alone. 08-27: `feed 2041 · snapshotted 2041 · moves_detected 6 · delisted 0 · trusted_prior true · overlap 1.0 · prior_age_days 1 · errors null` | 2026-08-27 | pricing-history curl |
| **HOW THE CAPTURE ACTUALLY RUNS — do not re-open the 02:20 skips as a failure** | The Vercel `pricing-history` cron at **02:20 UTC always skips** (`stale feed — deployed book predates today`) because the feed workflow does not land until ~02:50. That is expected. The REAL capture is the feed-refresh workflow's own step, which **polls `/api/cron/pricing-history` up to 30× at 30s** until the route reports the generation date of the book it just pushed, then asserts `snapshotted>0` and a non-empty diff baseline and **exits 1** otherwise. My morning curl is idempotent belt-and-braces (`moves_already_logged`). **As of `12df144` there is now a THIRD leg:** a 12:00 UTC Vercel cron that turns a still-stale book into a logged `error` rather than another quiet `skipped` | 2026-08-27 | `cron_logs` + `.github/workflows/feed-refresh.yml` |
| Snapshot rows by day | 2,035 (08-23), 2,035, 2,036, 2,036 (08-26), **2,041 (08-27)** — one clean write per day, rows = distinct refs every day | 2026-08-27 | `price_snapshots` |
| Delistings | **0 new tombstones 08-27.** Cumulative **88** | 2026-08-27 | `sold_properties` |
| **NIGHTLY RELIABILITY — MEASURE EXISTENCE, NOT CONCLUSION** | **2026-08-27: GitHub dropped BOTH nightly workflows (O-60).** Recovered by hand; the day is complete. Scheduled-run existence: 08-14..08-26 present and successful (13), **08-27 ABSENT**. Vercel crons unaffected that night (32 runs / 19 jobs by 05:55, vs 31/20 the day before) | 2026-08-27 | `actions_list` + `cron_logs` |
| Build health | Feed refresh run 31 (dispatch) **success**, all steps; IndexNow run 15 (dispatch) **success**. No open PRs. One push to main today (`12df144`); all four gates green locally before push | 2026-08-27 | `actions_list` |
| **CRAWLER LEDGER** | `crawler_hits` **begins 2026-08-11 11:46** (no earlier data — why the robots.txt experiment was unmeasurable). Full days to 08-26: ClaudeBot 2,098 (08-24) → 1,409 (08-25); AwarioBot #1 at 3,535 / 4,864; Googlebot 1,315 / 2,242. **Not re-queried today — the capture emergency took the time, and a partial day cannot be compared to a full one anyway.** Re-derive tomorrow | 2026-08-26 | `crawler_hits` grouped by day |
...
| **Citation rate, organic (qb-v2) — THE baseline** | **7.35% (5/68) on 08-26.** Eight complete runs: 4.41 (08-10), 4.41, 2.94, 5.88, 8.82, 5.88, 7.35 (08-24), 7.35 (08-26). Mean **5.88%**. One hit = 1.47pp. **No detectable trend. Do not claim one.** | 2026-08-26 | `citation_measurements` |
...
| Citation run coverage | Mon/Wed/Fri. **08-27 is a Thursday — no run due, and none expected.** Last: Wed 08-26, 68/68 organic + 6/6 branded, MEASURED. Next: **Fri 08-28** | 2026-08-27 | `vercel.json` crons |
...
| APCI macro input age | **96 days** (`as_of` 2026-05-23) — climbing daily until O-34/O-40 resolved | 2026-08-27 | `/api/v1/apci` |
| Cron success rates (worst, among those that log) | `counterpart-discover` failing daily (again 08-27 03:30) · `eu-stats-ingest` failing daily (again 08-27 04:15) · `auto-post` 3×/day (O-53) · `prometheus` `error_count:4` (O-56) · `weekly-alpha`, `digest`, `generate-briefs`, `predictions/generate`, `pulse` all on the Anthropic balance | 2026-08-27 | `cron_logs` grouped |
