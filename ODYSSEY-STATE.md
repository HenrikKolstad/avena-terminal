...
|---|---|---|---|
| 2026-09-06 | `47fc0b4` **THE CAPTURE BACKSTOP — O-80 SHIPPED.** Separate workflow, separate schedule (07:00 + 09:00 UTC), checks whether today is captured and re-dispatches feed-refresh only if it is not | **READ OUT THE SAME DAY, IN PRODUCTION, ON THE HALF THAT COULD REFUTE ME — by dispatching it with `dry_run: true`, which makes the dangerous action unreachable by construction while exercising every real signal reader.** Run 1, 06:37:10, **green in 32s**: `RESULT action=no_op reason=snapshot_present degraded=false snapshot=captured book=captured in_flight=no dispatched=false`. **That is criterion (a) exactly.** **The part that could not be tested locally and IS now proven: all three readers worked against real production credentials — the Supabase read found today's 2,034 rows, the checkout gave the 09-06 book, the Actions API answered — and NONE fell back to `indeterminate`.** The embedded test step ("Prove the decision rule before trusting it") also passed in CI, so a future change that breaks the rule cannot reach a dispatch. **WHAT IS STILL NOT VERIFIED, stated precisely so I do not later claim more than I proved: (1) the DISPATCH decision path, which needs a genuine nightly failure — three in ten days, so expect one within a fortnight; (2) `dispatchFeedRefresh`'s actual POST, which `dry_run` deliberately skipped and which has therefore NEVER been executed. If that POST is malformed or the token lacks `actions: write`, the backstop would decide correctly and then fail — LOUDLY (it exits 1), but it would fail.** **Tomorrow: confirm the 07:00 and 09:00 SCHEDULED runs behave as this dispatched one did — a workflow that runs on demand is not proof its cron fires** | **VERIFIED SAME-DAY on the no-op half; dispatch half awaits a real failure** |
