...
| 2026-09-01 | `908be3a` **`snapshot_superseded` on pricing-history** | **FOURTH consecutive correct negative:** `snapshot_superseded: 0, refs: null` on a clean single-book day (09-04, feed 2033/snapshotted 2033, overlap 0.999). Still no natural positive. **Keep reading this field in daily sense 3 — the first non-zero is the real read-out** | **NEGATIVE VERIFIED ×4; positive pending a natural occurrence** |
...
| O-61 | **GitHub's scheduler is degraded, DAY 9.** Today's capture was hand-dispatched again (run 52, `workflow_dispatch`, 05:37:33, **fully green through all 14 steps**, 2,033 listings). **NEW AND IT CUTS BOTH WAYS: 09-03 produced TWO scheduled runs that both SUCCEEDED — run 50 at 06:24 and run 51 at 09:40 — and 09-02's 06:21 run also succeeded.** So the scheduler is now *late* rather than *absent*, three days running | `actions_list`; run 50/51/52 step timelines | **Per the standing correction I do NOT call today's absence at 05:37.** **But I must be honest that the dispatch's marginal value has fallen:** on 09-02 and 09-03 a scheduled run landed within ~45 min of my dispatch and would have captured the day anyway. **What kept it safe is that all three of 09-03's books were byte-identical in `data.json` — so three downloads produced one book, not a union.** The dispatch stays until `GITHUB_DATA_TOKEN` lands, because a 45-minute margin is not a system, but **the honest framing is "redundant on the last two days", not "saved the day nine times".** **No day lost: 08-27..09-04, all nine captured** | **HIGH — day 9** |
...
**No new experiment today — NINTH consecutive day, and no read-out was due
either (next 09-08).** The reason is unchanged and still right: my experiment
...
**THE RISK TO THE EXPERIMENT LEDGER IS ACUTE, DAY 5.** Perplexity has been out
since 08-31. **09-04 WAS a run day and it failed again: all three atlas
invocations 401'd on 74 of 74 lookups.** The engine has now measured nothing
since **08-28 — seven days.** **Four pending read-outs (09-08 ×2, 09-09 ×2) are
measured by an engine that is dark, and the first two are FOUR DAYS AWAY.** If
...
|---|---|---|---|
| AVM median absolute error | **15.89%** (in-sample, n=**2,033**), MAPE 21.95%. Gate run reproduced the committed file exactly apart from `computed_at`, reverted as churn (sixth day running). **The move 15.82 -> 15.89 is the BOOK changing (2,034 -> 2,033), not a model change — no AVM code was touched** | 2026-09-04 | `public/model-stats.json` |
| Live book | **2,033 listings** (was 2,034) | 2026-09-04 | `public/data.json`, feed commit `5837014` (**manual dispatch, run 52, 9th day running; fully green through all 14 steps**) |
| Sitemap | **2,685 `<loc>`** (unchanged), valid XML, 5 sampled property URLs all 200 (SP0518, N9532, N9969, SP1804, N3099V) | 2026-09-04 | `/sitemap.xml`, parsed |
| Frontier sitemap | **137 property URLs** (was 130, 120, 117, 116, 118, 121, 122, 127) (O-59) | 2026-09-04 | `/sitemap-frontier.xml`, parsed |
| Corpus version | site **v2026-09-04** (= today ✓, generated 05:42:12Z) · `avena-data` mirror at **v2026-09-03** (its normal lag) · HF unverified (401 without a token) | 2026-09-04 | site + mirror raw |
...
| **Real price moves by day** | 15 (08-14), 4, 1, 0, 15, 10, 10, 18 (08-21), 9, 0, 0, 3, 6, 5, 6 (08-28), 6, 0 (08-30), 1 (08-31), **7 (09-01)**, **12 (09-02)**, **16 (09-03)**, **19 (09-04, provisional)**. **Definition now fixed: a move is counted only when the previous snapshot is ≤2 days earlier.** On that definition 09-01 is **6**, not 7 | 2026-09-04 | `price_snapshots`, direct SQL diff |
...
| Snapshot rows by day | 2,047 (08-28), 2,042, 2,044 (08-30), **2,044 (08-31 — UNION, true final book 2,042, O-74)**, 2,043 (09-01), 2,033 (09-02), 2,034 (09-03), **2,033 (09-04)** | 2026-09-04 | `price_snapshots` |
| Today's capture | feed 2,033 · snapshotted 2,033 · moves_detected 19 · route `delisted: 0` but **3 real tombstones by parse-feed** · **snapshot_superseded 0** · prior 09-03, age 1d, trusted · overlap **0.999** · errors null · scribe 2,033/2,033 accepted, 0 failed, `book_lag_days: 0` | 2026-09-04 | `/api/cron/pricing-history` |
| Delistings | **3 new tombstones today (N9468, N8500, SP0385), ALL stamped `last_seen_date` 09-04 when last truly seen 09-03 — all 3 wrong.** Cumulative **117: 31 correct · 76 one day late · 10 stamped behind = 86 wrong (73.5%)** (was 73%, 62%, 51%) | 2026-09-04 | `sold_properties`, re-derived |
...
| Build health | **Run 52 (dispatch, 09-04 05:37:33) success — all 14 steps green**, book committed `5837014`, corpus `0fac21b`. **09-03 also produced TWO successful SCHEDULED runs (50 @ 06:24, 51 @ 09:40)** — the scheduler is late, not absent. No open PRs. **One push to main today** (`141bf2e`); all four gates green before it. No red checks on any branch | 2026-09-04 | `actions_list` |
...
| **THE ANTHROPIC API BALANCE IS EXHAUSTED — degrading seven jobs** (standing, day 13) | `predictions/generate`, `digest`, `generate-briefs`, `weekly-alpha` error on "credit balance is too low"; `delphi-run` and `plab-run` skip the Claude panelists (`models_scored: []`); `pulse` fails HTTP 500. `sync-regulatory-signals` fails classification for this reason, which is why `regulatory_signals` has ingested nothing since 08-04. | **A decision, not a task: top up or don't.** If you top up, `predictions/generate` starts publishing LLM-authored forecasts on `/track-record` — the class of surface that produced the `precursor-scan` fabrication, so **say so explicitly if you want that live**. If you don't, tell me and I'll make the affected routes report `skipped` with a stated reason instead of failing nightly. **The quieter harm: DELPHI and PLAB publish a "panel" consensus that is now, on some days, no models at all.** |
