...
|---|---|---|---|
| 2026-09-10 | `d2a52ea` **O-82/CBS — the last silent-zero adapter. Four stacked defects: a DISCONTINUED table (83913NED, `Frequency: Stopgezet`), a `valueField` that does not exist in the payload, `$orderby` silently ignored (so `$top=80` meant "the oldest 80"), and no region pin across 21 RegioS values.** Repointed to 85792NED, field → `PrijsindexVerkoopprijzen_1`, region pinned to `NL01  ` AND VERIFIED against the returned rows, every zero-row path now THROWS | **THE DISCRIMINATING CHECK IS THE 04:15 RUN, AND IT HAS THREE OUTCOMES I MUST KEEP APART — I could not exercise the production fetch path from the sandbox (direct egress gets HTTP 406 from opendata.cbs.nl on EVERY url, including the bare table root and the old table; only the agent proxy gets 200), so production reaching CBS is INFERRED, not observed.** (a) **SUCCESS**: `eu_official_stats` source `cbs` goes 0 → **157 rows** (126 quarterly + 31 annual, 1995-Q1 through 2026-Q2), sources 4 → **5**, total 17,525 → **17,682**. (b) **HONEST FAILURE**: cbs appears in `cron_logs.error` with a real message (`HTTP 406`, `missing field …`, `region filter not applied`, `none usable`). **That is a PASS for this commit even though the adapter stays empty — the entire point was to make the failure speakable. Record it as "guard works, upstream/egress blocked", NOT as "the fix failed".** (c) **THE ONLY REAL FAILURE**: cbs is still 0 rows AND still absent from `errors`. That would mean a zero-row path I did not close. **Do not collapse (b) into (c).** Also confirm `errors` count: 1 (istat) → 1 or 2, never silently back to 1-with-cbs-empty | **pending — read out 2026-09-11** |
| 2026-09-10 | `9053781` **the OpenAPI spec served a flatly false claim about the largest EU stats source.** BIS held 8,700 rows from 04:15 (49.6% of the table) while `/api/openapi.json` named three sources and asserted "Any adapter not listed here is wired but is not returning rows". Observation now DATED, dormancy assertion removed, `revalidate` 86400 → 3600 | **VERIFIED IN PRODUCTION THE SAME DAY, by re-fetching and parsing the served spec.** Enum reads `['bis','ecb_sdw','eurostat','ine_es']`; the description reads "Sources holding observations as of 2026-09-10T05:46:00.918Z: BIS, ECB SDW, Eurostat, INE Spain" and states the cache window; the `source` param carries its own dated caveat. **TOMORROW: the TRACKING half, which is the half that can still fail.** The timestamp must MOVE (a frozen `05:46:00.918Z` means I shipped a value that renders once and caches forever), and if `cbs` lands per the row above the enum must become FIVE entries **without my touching this file** — that is the same "derived value vs. literal wearing a function's clothes" test that O-83 and `/engine` both had to pass | **VERIFIED same-day on the content; the TRACKING half reads out 09-11** |
| 2026-09-09 | `10690b1` **O-82/BIS — the adapter was 404 AND could not have reported a layout change.** URL repointed to the SDMX portal (`stats.bis.org/api/v1/data/WS_SPP/all/all?format=csv`); parse extracted to the pure, exported `parseBisSppCsv`; columns addressed BY NAME; **zero usable rows from a non-empty body now THROWS** | **The discriminating check is NOT "did bis get rows" — it is whether the two derived surfaces move WITH it, because either could be re-hardcoded without my noticing.** (a) `eu-stats-ingest` 09-10 04:15 must report **bis ≈ 8,700 rows** and `errors` down from 2 to **1** (istat HTTP 500 only — bis's 404 must be gone from the error string). (b) `eu_official_stats` total **8,825 → ≈17,525**, sources 3 → **4**, distinct countries **28 → ~57+**. (c) **`/api/openapi.json`'s `/api/v1/stats` `source` enum must list FOUR sources including `bis`.** It listed exactly three today (`ecb_sdw, eurostat, ine_es`), verified live — so a frozen three tomorrow means I replaced a literal with a query that does not track, which is the O-83 failure one surface over. (d) Run duration must stay well inside 265s; today's was 39.1s and BIS adds a 1.3MB fetch plus ~18 chunk writes. **If bis lands but (c) still reads three, the ADAPTER is fixed and the SPEC is not — record them separately, do not collapse them** | **READ OUT. THREE MET; (c) FAILED EXACTLY AS PRE-REGISTERED, AND PRE-REGISTERING IT IS THE ONLY REASON I LOOKED.** (a) **MET**: 09-10 04:15 scheduled run, `rows_upserted` **17,525**, `errors` 2 → **1** — `istat HTTP 500` alone, the BIS 404 gone. (b) **MET**: `eu_official_stats` 8,825 → **17,525**, sources 3 → **4**, `bis` **8,700 rows across 57 countries** — now the largest source in the table. (d) **MET**: 46.1s of a 265s budget (04:15:29 → 04:16:15). (c) **FAILED**: `/api/openapi.json` still served `enum: ["ecb_sdw","eurostat","ine_es"]`. **And the split I wrote the criterion to detect turned out to be finer than the two cases I named: the spec CODE already derived correctly — the failure was that it is an ISR render cached 24h, publishing a stale snapshot in the PRESENT TENSE plus a sentence asserting the unlisted adapters return no rows. So the spec did not merely omit the largest source, it DENIED it.** Fixed today in `9053781`. **Recorded as: adapter VERIFIED on all three of its own criteria; the surface it feeds was a separate defect that my criterion caught and my two-case description of it did not anticipate** |
| 2026-09-09 | `70e2d85` **/engine's meta description — all four figures were false.** `generateMetadata()` derives savings + underpriced from `getEngineStats()`; the two DB-backed figures deleted, not replaced | **VERIFIED IN PRODUCTION THE SAME DAY, by parsing the served payload.** `396,000` ×0 · `394,000` ×0 · `265M` ×0 · `1,425` ×0; derived `€278M` and `1,464` both present. The ~9x transaction overstatement is off the page. **TOMORROW: the TRACKING half — after tonight's feed the served figures must MOVE with the book (1,464/€278M are today's values on a 2,037 book). A frozen 1,464 would mean I shipped a literal wearing a function's clothes.** Also confirm `/engine` still 200 (the metadata export became async) | **FULLY VERIFIED — the tracking half landed and it is the half that mattered.** The book moved **2,037 → 2,036** overnight and the served meta description moved with it: **€278M across `1,463` underpriced homes** (was 1,464 on a 2,037 book). All four false figures still absent — `396,000` ×0, `394,000` ×0, `265M` ×0, `1,425` ×0. `/engine` 200. **A frozen 1,464 was the discriminating failure and it did not happen — the figures genuinely derive → CLOSED** |
| 2026-09-08 | `08272fc` **O-83 — the book's size was a literal on ~40 surfaces** | **FULLY VERIFIED — the tracking half landed today and it is the half that mattered.** The book moved **2,034 → 2,037** overnight, and the live homepage now serves **`2,034` ZERO times and `2,037` five times**. Both JSON-LD blocks re-parsed: `WebSite` → "Depth: **2,037** scored Spanish new-builds", `Dataset` → "Scored dataset of **2,037** new-build properties". **A frozen 2,034 was the discriminating failure and it did not happen — the count genuinely derives.** `/deals`, `/no`, `/engine`, 5 random `/property/{ref}` all 200 | **VERIFIED ON BOTH HALVES → CLOSED (moved to section 6; do not re-verify)** |
| 2026-09-08 | `1745d91` **`shareBudget` — two hung ECB indicators starved a healthy third** | **READ OUT. THREE CRITERIA MET, ONE UNEXERCISED — and I am recording the unexercised one as unexercised rather than rounding it up, which is the whole point of pre-registering criteria that can fail independently.** (a) **MET**: `rows_upserted` **8,825** on the 04:15 SCHEDULED run with no hand re-run (yesterday 8,623). (c) **MET, and this was the change's own amplification risk**: `eurostat` exactly **3,808** and `ine_es` exactly **4,480** — the fair-share slices did NOT starve the healthy sources. (d) **MET**: **39.1s** against a 265s budget (yesterday 99.2s). (b) **UNEXERCISED**: no indicator reported `budget exhausted before attempt 1/3` — but **ECB was HEALTHY today** (the 39s duration and the absence of any timeout in `error` both say so), so the starvation path never ran. **This is NOT evidence the fix works; it is the absence of an occasion to test it.** The only errors were the two known upstream ones: `istat HTTP 500 \| bis HTTP 404` | **3 of 4 MET; (b) STILL UNEXERCISED on day 2 — the 09-10 04:15 run was healthy again (46.1s, errors=1, no `budget exhausted` line), so the starvation path has still never run. Absence of an occasion, not evidence of a fix. Re-registers for the next ECB hang** |
| 2026-09-04 | `141bf2e` **`fetchCommuneYear` throwing on a dead upstream** | **Negative still bounded** (28/28 commune-years return 200, so it cannot false-alarm); **the positive still awaits a natural upstream outage.** Tell: `dvf-ingest` logging `error` with `DVF fetch failed: HTTP …` instead of `fetched: 0` + `success` | **NEGATIVE BOUNDED; positive awaits a natural outage (day 6 — `dvf-ingest` succeeded again 09-10 04:30)** |
...
| O-88 | **NEW — AwarioBot has escalated 3.5x and is now the largest consumer of the origin by a wide margin.** **30,016 hits over 7 days across 2,276 paths** — **13.2 fetches per path per week** — against Googlebot's 10,935 hits / 5,411 paths | `crawler_hits`, 7-day window, 09-09. O-14 recorded 8,559 hits / 1,988 frozen paths on 09-01 | **This is O-14 escalating, not a new finding, and its path count DID move this time (1,988 → 2,276), roughly tracking the book — so it is no longer "re-fetching a frozen URL set". It is fetching the right set ~13x more often than anything could need.** Awario is brand-monitoring SaaS: it contributes nothing to search or AI citation. `robots.ts` currently gives it `BROAD_ALLOW`. **Deferred deliberately today, and I want the reason on record because it is a judgement Henrik may disagree with: this is compute and cost, NOT correctness or reach, so it ranks below the two credibility items I shipped. It is also a one-line, instantly reversible `Disallow` with a real pre-change baseline (unlike the 08-11 robots.txt experiment, which had none), so it is the cleanest small SEO-adjacent change available.** Do it on the next quiet day | medium |
...
| O-62 | **Absorption ledger delisting dates — 138 tombstones: 31 correct · 94 one day LATE · 13 stamped behind = 107 wrong (77.5%).** Re-derived by direct SQL this morning; it grew 132 → 138 and **ALL SIX new departures are wrong — `correct` did not move off 31** | direct SQL 09-10: `max(snapshot_date)` per ref vs `sold_properties.last_seen_date` | **77.5%, up from 76.5% / 76.2% / 73.5% / 73% / 62% / 51%. The mechanism is confirmed for the seventh day: `parse-feed.js` (the second writer, O-20) stamps the day it NOTICED, not the last day the unit was in the book. Branch `odyssey/absorption-ledger-dates` has now waited TWENTY-FOUR days.** **The number worth staring at: `correct` has been frozen at 31 for three days while the total climbed by 8. Not one departure since 09-07 has been stamped right, so the error rate is not drifting up — every single new row is wrong, and the 31 are historical.** Every new departure makes the backfill larger and the release later. I will not compute a Release 1 delisting-by-day slot from this table | **HIGHEST of the open items — overdue by 4 days** |
...
| O-90 | **NEW — the pattern behind today's OpenAPI bug is bigger than the one route: a DERIVED value rendered into a CACHED document is a snapshot, and every one of them is written in the present tense.** `/api/openapi.json` had `revalidate = 86400` and said "Sources CURRENTLY holding observations" | measured 09-10: BIS held 8,700 rows from 04:15 while the served spec named three sources and asserted the rest returned none. `x-vercel-cache: HIT` | **Fixed on that route (`9053781`), but I have NOT swept for siblings and I am recording that as unfinished rather than implying the class is closed. The grep that pays: any `export const revalidate` or `s-maxage` on a route whose body derives a COUNT, a LIST or a STATUS from Supabase — then read whether its prose is tensed as current.** `/defensibility` and `/eu-official` both derive per request and are fine (checked today: `/eu-official` rendered BIS 242× on the same page load the spec was denying it). **The general rule worth keeping: deriving a value fixes WHERE it comes from, not WHEN it was true. Cache + present tense = a false claim on a timer** | medium-high |
...
**No new experiment today — FIFTEENTH consecutive day, same reason and it has
not weakened.** Both changes I shipped are truth corrections: a machine-readable
...
(1) **The measurement instrument is dark.** The Perplexity balance is out on
**day 11**. Today (Thu) is NOT a run day — `atlas` is correctly absent from the
last 26h of `cron_logs`, and `cassandra` logged `2026-09-09
raw_rows_absent_on_a_run_day`, which is the `908be3a` discriminating pair
working for a fifth consecutive day. **Five read-outs have now resolved to
UNMEASURABLE for want of a paid balance.** Starting a sixth experiment measured
by that instrument would be theatre. **Next run day is Fri 09-11 — if the
balance is still out it will fail identically, and that is a repeat, not news.**
...
**Next read-outs: 09-11 (`d2a52ea` CBS first run + `9053781` spec-timestamp
tracking), 09-14 (O-6 / CompareLedgerPulse — needs Henrik's GSC Generative-AI
...
|---|---|---|---|
| AVM median absolute error | **15.75%** (in-sample, n=**2,036**), MAPE 21.76%, RMSE 42.93%. **My gate run reproduced the committed file EXACTLY, twice — 15.75% / n=2,036.** Same-book comparison again (the 09-09 feed landed 09:38 and regenerated it). No AVM code touched; today's two commits are an OpenAPI spec and an ingest adapter | 2026-09-10 | `public/model-stats.json` |
...
| **ENVIRONMENT FACT — the session starts in DETACHED HEAD, every single morning** | `git status -sb` reads `## HEAD (no branch)` and the local `main` ref lags origin (yesterday by 31 commits; **today by 5**). Pushing without fixing it sends the STALE ref and is rejected as non-fast-forward with a message that reads like a race and is not one. **The fix is `git branch -f main <origin sha> && git checkout main`, NOT a force-push.** **Applied FIRST today, before any work, and both pushes went through clean on the first attempt — this row paid for itself** | 2026-09-10 | `git status -sb`, `git branch -v` |
| Live book | **2,036 listings** (09-09 feed, committed 09:38), down from 2,037. **Today's 09-10 feed had NOT landed at 05:37** — see the capture row | 2026-09-10 (pre-run) | `public/data.json` |
| Sitemap | **2,687 `<loc>`**, valid XML, HTTP 200, 433KB. Moved 2,689 → 2,687 with the book (2,037 → 2,036), which is the right behaviour. 5 sampled URLs all 200 (SP0556, N9460, /compare/benahavis-malaga-vs-san-javier-murcia, N7792, SP0996) | 2026-09-10 | `/sitemap.xml`, parsed |
| Corpus version | **TWO-WAY AGREEMENT: site `v2026-09-09` = mirror `v2026-09-09`**, identical `generated_at` (2026-09-09T09:38:38.605Z). Measured 05:38, before today's rebuild — so `v2026-09-09` on 09-10 is the EXPECTED state, not staleness. **HF remains 401 (no token) and is still the ONLY unverified surface** | 2026-09-10 | site + avena-data raw |
...
| **`eu_official_stats`** | **17,525 rows, FOUR sources: `bis` 8,700 (57 countries) · `ine_es` 4,480 (1) · `eurostat` 3,808 (28) · `ecb_sdw` 537 (7).** The table roughly DOUBLED overnight and `bis` is now the largest source in it. `istat` holds ZERO (upstream HTTP 500). **`cbs` holds zero TODAY and must hold 157 TOMORROW — `d2a52ea` repaired it. Expected 09-11: 17,682 rows, 5 sources. If cbs is still 0 AND still absent from `errors`, the fix did not land and that is the day's top item; if cbs is 0 WITH a named error, the guard worked and the blocker is upstream or egress** | 2026-09-10 | direct SQL |
| **Ingest write funnel — eu-stats-ingest** | **09-10 04:15 SCHEDULED: `rows_upserted` 17,525 · `rows_lost` 0 · `write_chunks_failed` 0 · `rows_duplicate_excluded` 0 · `rows_undecodable` 0 · `indicators_attempted` 20 · errors 1 · duration 46.1s.** **Doubled the row count and still used only 46s of a 265s budget — BIS's 1.3MB fetch plus ~18 chunk writes cost ~7s.** **The 1 error is `istat HTTP 500` alone; the bis 404 is gone.** Pre-`3a55753` this ran at `upserted 4,345 / lost 4,480` every night for months | 2026-09-10 | `cron_logs` |
...
| Snapshot rows by day | 2,034 (09-03), **2,037 (09-04 — TWO writes, clean superset)**, 2,026 (09-05), 2,034 (09-06), 2,034 (09-07), **2,037 (09-08)**, **2,037 (09-09 — TWO writes, union day, see O-74)** | 2026-09-10 | `price_snapshots` |
...
| **TODAY'S CAPTURE (09-10) — NOT YET LANDED AT 05:37, AND THAT IS NOT A FAILURE** | The 05:10 slot has landed 06:15–06:43 for nine nights (65–93 min queue lag). I finished checks at 05:37, so the run did not yet exist. `pricing-history` returns `skipped · stale feed — deployed book predates today · feed_age_days 1 · overdue FALSE · feed 2036` — the documented healthy 'not yet' state, NOT the zero-snapshot emergency (which is a skip or zero on a day the feed HAD refreshed and deployed). **Standing rule, applied without re-learning it for a second consecutive day: any observation of the GitHub scheduler before ~11:00 UTC is worthless as evidence of absence.** The backstop (`47fc0b4`) is armed; its 07:00/09:00 slots land ~11:00–13:15 | 2026-09-10 05:37 | `cron_logs`, `git log origin/main` |
...
| Delistings | **132 tombstones: 31 correct · 89 one day late · 12 stamped behind = 101 wrong (76.5%)** — re-derived 09-09. **Grew from 130 and BOTH new departures are wrong.** Unmet gate under Plan B Release 1, whose fire date (09-07) passed without firing | 2026-09-09 | `sold_properties` × `price_snapshots` |
| Build health | **ALL GREEN. Last 18 workflow runs across feed-refresh, capture-backstop, indexnow, pulse-alerts and pulse-weekly are `success`; no red check on any branch; no open PRs.** **Two pushes to main today (`9053781`, `d2a52ea`), all four gates green before each.** Reds in `cron_logs` over 26h are ALL known and attributed: prometheus ×4 + generate-briefs + predictions-generate + causal-update + pulse (Anthropic balance), auto-post ×3 (O-53), cassandra ×2 (CORRECT — reporting 09-09 as a run day with no measurement), eu-stats-ingest ×2 (the 09-09 run with bis; the 09-10 run with istat alone), counterpart-discover (O-41). **`atlas` is correctly ABSENT — 09-10 is a Thursday, not a run day.** **Nothing new or unexplained** | 2026-09-10 | `cron_logs`, `actions_list` |
| Test coverage added by Odyssey | `test-open-dataset` 27 · `test-scribe` 22 · `test-cron-coverage` 99 · `test-integrity` 15 · `test-capture-integrity` 19 · `test-chunked-write` 71 · `test-prometheus-reporting` 25 · `test-capture-backstop` 37 · `test-eu-stats-keys` 40 · `test-resilient-fetch` 46 · `test-corpus-count` 14 · `test-bis-parse` 32 · **`test-openapi-stats-sources` 19 (new)** · **`test-cbs-parse` 35 (new)** | 2026-09-10 | `9053781`, `d2a52ea` |
...
| **Crawler ledger, 7 days** | **AwarioBot 30,016 hits / 2,276 paths (O-88, and 3x Googlebot)** · Googlebot 10,935 / 5,411 · PetalBot 6,593 / 4,069 · AhrefsBot 1,702 · Amazonbot 1,457 · bingbot 1,343 / 486 · SemrushBot 1,027 · ClaudeBot 562 / 99 · Applebot 339 · **OAI-SearchBot 292 / 123** · **ChatGPT-User 246 / 47** · Bytespider 153 · **PerplexityBot 63 / 53** · meta-externalagent 61 · **GPTBot 40 / 9** | 2026-09-09 | `crawler_hits` |
...
| **CITATION ENGINE — DARK SINCE 2026-08-31, DAY 11; LAST REAL MEASUREMENT 08-28, THIRTEEN DAYS** | **09-10 IS a Thursday and is NOT a run day** (derived from a calendar, not asserted), so `atlas`'s absence from the last 26h is CORRECT and must not be read as a new failure. `cassandra` ran 04:15 and logged `2026-09-09: raw_rows_absent_on_a_run_day` — **the `908be3a` discriminating pair working for a fifth consecutive day: it names the run day that produced nothing instead of silently averaging over it.** **The `9171dce` guard is holding: no fabricated 0.00% has been published in eleven days.** **Next run day: Fri 09-11.** **`plab-run`'s `keys: {perplexity: true}` means the env var is SET, not that it has credit** | 2026-09-10 | `cron_logs` |
...
| APCI macro input age | **109 days** (`as_of` 2026-05-23) — climbing daily until O-34/O-40 resolved | 2026-09-09 | `/api/v1/apci` |
