...
|---|---|---|---|---|
| O-23 | **29 of 74 citation lookups failed on 2026-08-12 and the cause is unknown.** The agent handled it correctly (failures excluded, not zeroed) and `24db855` now records the coverage, but neither tells us WHY 39% of the bank failed. The prime suspect is the Perplexity balance running out mid-run, which would be a NEEDS HENRIK item — but the failure reason lives only in Vercel's ephemeral logs for 03:04 UTC and I have not seen a 401. **Do not assert the balance is out without that evidence.** | `citation_measurements` 08-12: 42/68 organic, 3/6 branded; 29 questions present on 08-10 and absent on 08-12, 0 the other way | needs the run's actual error string. Either read Vercel logs for the 03:00 UTC cron, or persist `first_error`/`lookups_failed` from the agent into the rollup so the next partial run explains itself | high |
