...
|---|---|---|---|
| 2026-08-24 | `fde7883` **/api/v1/tax stopped asserting a 7%/yr forecast and a 5.5% default yield** | **VERIFIED LIVE the same day.** No-scenario body → `projected_exit_price` null and `estimated_gross_yield` null (not 5.5%). Scenario body (`annual_appreciation_pct:3, gross_yield_pct:5`) → `projected_exit_price` 403175 (compound), `appreciation_source:"caller_supplied_scenario"`, `yield_source:"caller_supplied"`. Deploy confirmed on prod ~1 min after push | **VERIFIED — see CLOSED** |
