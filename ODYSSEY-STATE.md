...
| 2026-08-12 | `2416532` **Market Pulse delivery engine** — `scripts/pulse/generate_editions.py` + `.github/workflows/pulse-weekly.yml` (cron Mon 05:45 UTC). Henrik sent 30 cold outreach emails today selling a €500/mo weekly PDF ("first edition arrives Monday 08:00 CET") — this engine IS that promise. Subscribers live in `pulse_subscribers` (towns[] per row); deliveries logged in `pulse_deliveries` with UNIQUE(subscriber, edition_date) so re-runs fill gaps, never double-send. Sends via Resend from pulse@avenaterminal.com. GitHub secret `RESEND_API_KEY` set. Gotcha: api.resend.com sits behind Cloudflare and 403s (error 1010) any default `Python-urllib` UA — the script sends `avena-pulse/1.0`. | Monday 2026-08-17 05:45 UTC is the first scheduled fire. Verify the Actions run went green and `pulse_deliveries` has a row per active subscriber for 2026-08-17. End-to-end already verified manually 08-12 (subscriber #1, resend 18bd97c7). **If a Stripe payment lands, a subscriber row (email + towns) MUST be added before Monday** — Henrik or Fable does this; flag loudly in the brief if a payment exists with no matching row. Same-day reprice alerts are promised in the outreach but NOT built yet. | pending — first cron fire 2026-08-17 |
...
| ~~O-19~~ | **CLOSED 2026-08-12 — merged and applied by Fable on Henrik's explicit delegation.** Reviewed line by line: drops exactly one constraint (the FK rejecting 100% of live refs, carrying the CASCADE that would have deleted the 394k-row history) + one index. Applied to prod, branch merged to main. **Same-day proof: the capture re-run wrote the first move events in the table's life — `price_moves:15`, `moves_ledger_blocked:null` — and all 15 `increased` rows match the price_snapshots ground-truth diff exactly (15 ups, 0 downs).** | migration `move_ledger_accept_live_refs`; `property_pricing_history` now holds 15 real events | — | closed |
...
| 2026-08-12 | Exposing the observation ledger as MCP tools turns Avena from a site AIs READ into a source AIs USE — "has this property cut its price?" is now answerable by an agent, and only via us | MCP tools 8–11 (`get_price_history`, `get_recent_price_moves`, `get_delistings`, `get_avena_index`) + `mcp_calls.tool` column logging WHAT each agent asks. Existing 7 tools untouched; verified live same day (SP1514 history, 12 Estepona moves, 33 delistings, index 1605.98) | `mcp_calls` grouped by tool: do external (non-Henrik) callers appear, and which ledger tools do they reach for? The query stream is the read-out | 2026-09-09 (4 weeks) | pending — needs distribution: server not yet listed in any MCP registry/directory |
| 2026-08-12 | **Nightly Quotable**: one extractable sentence (number+AVENA+date) + fan-out Q&A passages on all 97 town pages, regenerated nightly, Speakable-marked — exploits citation/rank decoupling + the 7-day freshness window | `TownLedgerPulse` component, verified live (Estepona: "12 of 231... +5.0% — per the AVENA observation ledger, 12 August 2026") | qb-v2 citation benchmark: organic rate vs 4.41% baseline; Perplexity/AI Overview citations of town pages specifically | 2026-09-09 (4 weeks) | pending |
| 2026-08-12 | **/statistics hub**: 18 dated branded stat sentences, nightly regenerated — the passive "X statistics" citation flywheel | live, in sitemap (additive line) | rankings for "spanish property statistics" queries + inbound citations of the page; GSC impressions | 2026-09-23 (6 weeks) | pending |
| 2026-08-12 | **IndexNow nightly ping** (2,106 URLs → Bing = ChatGPT's retrieval index) | `scripts/indexnow-ping.mjs` + 03:30 UTC workflow; first ping 200 OK | Bing indexation coverage (needs Henrik's Bing Webmaster claim) + ChatGPT-User/OAI-SearchBot hit growth in crawler_hits | 2026-09-09 (4 weeks) | pending |
| 2026-08-12 | Announcing `/sitemap-frontier.xml` (30-day observed-change frontier, 76 pages) in robots.txt steers crawl budget toward pages that changed — the pages worth re-reading | robots.ts +1 Sitemap line (Henrik's explicit yes; nothing else touched) | crawler_hits: do GPTBot/ClaudeBot/Meta-ExternalAgent start fetching sitemap-frontier.xml, and does the share of their hits landing on frontier-listed URLs rise vs the 2026-08-04..10 baseline? | 2026-08-26 (2 weeks) | pending |
...
|---|---|---|
| ~~`actions:write`~~ | **RESOLVED 2026-08-12 by Henrik + Fable.** Fine-grained PAT (repo `avena-terminal`, Actions read/write, expires ~2026-11-10 — renew before then) lives at `~/.config/odyssey/github-token` (chmod 600). **Use as:** `GH_TOKEN=$(cat ~/.config/odyssey/github-token) gh run rerun <id>` or `... gh workflow run feed-refresh.yml -R HenrikKolstad/avena-terminal`. Verified live: a dispatch with this token succeeded (run 31580968713) — the exact call that 403'd at 03:40. NEVER print, log or commit the token; read it inline only. | — |
