...
| **Real price moves by day** | **19 on 09-11 (single clean write).** 15 (08-14), 4, 1, 0, 15, 10, 10, 18 (08-21), 9, 0, 0, 3, 6, 5, 6 (08-28), 6, 0 (08-30), 1 (08-31), 6 (09-01), 12, 16, 19, 19 (09-05), 1 (09-06), 0 (09-07), **26 (09-09 — the largest single day in the series)**, **09-08 and 09-10 pending**. **Definition: a move counts only when the previous snapshot is ≤2 days earlier** | 2026-09-10 | `price_snapshots`, direct SQL diff |
| Snapshot rows by day | 2,026 (09-05), 2,034 (09-06), 2,034 (09-07), **2,037 (09-08)**, **2,037 (09-09 — TWO writes, union day, see O-74)**, **2,036 (09-10)**, **2,033 (09-11 — single write, clean, quotable)**. **NINETEEN consecutive days captured, no gap** | 2026-09-11 | `price_snapshots` |
...
| **TODAY'S CAPTURE (09-11) — LANDED AND COMPLETE. DAY 19.** | **The feed landed 06:35:30 (2,033 listings, down from 2,036) — inside the O-61 window (05:10 slot, 85 min lag), exactly as the standing rule predicts.** Production redeployed by 06:37 and `pricing-history` captured on the first non-skipped attempt: **`snapshotted` 2,033 of a 2,033 feed**, `trusted_prior: true`, `prior_date` 09-10, `prior_age_days` 1, `overlap` **0.997**, `errors` null. **`snapshot_superseded: 0` and a SINGLE write — this is NOT a union day, so 09-11's listing count IS quotable, unlike 08-31 and 09-09.** **19 real price moves**, all 19 `moves_already_logged` — i.e. `parse-feed.js` wrote them first, which is O-20's two-writer pattern behaving as documented. **`delisted: 0` from the route, and per O-20 that field is meaningless: `sold_properties` grew 139 → 145, so SEVEN units actually departed today.** Always reconcile against the table. **Confirmed in the DB: `price_snapshots` 09-11 = 2,033 refs / 1,018 distinct prices.** | 2026-09-11 06:40 | `cron_logs`, `price_snapshots`, `sold_properties` |
...
| Delistings | **145 tombstones: 30 correct · 102 one day late · 13 stamped behind = 115 wrong (79.3%)** — re-derived 09-11 AFTER today's capture. **Grew 139 → 145; ALL SEVEN of today's departures are wrong. AND `correct` DROPPED 31 → 30, which it has never done before — the 31 "historical correct" are not even stable, so the backfill target is not a fixed set.** Unmet gate under Plan B Release 1, whose fire date (09-07) passed without firing | 2026-09-11 | `sold_properties` × `price_snapshots` |
