...
|---|---|---|---|
| 2026-09-03 | `07fbf93` **four ingest paths could not report how many rows they lost** — the last three O-1 sites + both dvf-ingest loops, now on `src/lib/chunked-write.ts` | **TOMORROW 09-04, and the discriminating row is dvf-ingest, which writes daily and is the only one of the four that has actually been failing.** `select output_summary from cron_logs where agent_id='dvf-ingest' and started_at >= '2026-09-04'`. **Three things must ALL hold:** (a) `transactions_fetched >= transactions_deduped >= transactions_inserted` and `transactions_deduped - transactions_inserted == transactions_lost` **exactly** — the arithmetic is the whole point; (b) on a clean commune `rows_lost: 0, chunks_failed: 0` and the run logs **`success`** — this is the half that can refute me, because if the new `errors[]` plumbing turns a healthy ingest red I have shipped a false-alarm generator; (c) if any chunk does fail, `errors_total >= errors.length` and the run logs `error`. **Also check `eu-validation` and `eu-anomalies` still log `success`** — they were green before and nothing about their write behaviour changed | **PENDING — Fri 09-04** |
...
| 2026-09-01 | `908be3a` **`snapshot_superseded` on pricing-history** | **THIRD consecutive correct negative:** `snapshot_superseded: 0, refs: null` on a clean single-book day (09-03, feed 2034/snapshotted 2034, overlap 0.997). Still no natural positive. **Keep reading this field in daily sense 3 — the first non-zero is the real read-out** | **NEGATIVE VERIFIED ×3; positive pending a natural occurrence** |

## 2. OPEN — found, not yet fixed

| # | what | evidence | why deferred | priority |
|---|---|---|---|---|
| O-62 | **Absorption ledger delisting dates — RE-DERIVED TODAY AND WORSE AGAIN.** **114 tombstones: 31 correct · 73 one day LATE · 10 stamped behind.** 7 new departures today, **all 7 stamped `last_seen_date` 2026-09-03 when they were last actually in the book on 09-02** | direct SQL 09-03: `max(snapshot_date)` per ref (`price_snapshots.ref`) vs `sold_properties.last_seen_date` | **The error rate is now 73%, up from 62% yesterday and 51% the day before. It grows by the daily delisting count and it will not stop on its own.** Mechanism confirmed for the third day: `parse-feed.js` (the second writer, O-20) stamps the day it NOTICED. **Branch `odyssey/absorption-ledger-dates` has been waiting 18 days. Plan B Release 1's window closes TOMORROW, 09-04, and Release 1 quotes delistings by day** | **HIGHEST of the open items** |
...
| O-61 | **GitHub's scheduler is degraded, DAY 8.** Today's capture was hand-dispatched again (run 49, `workflow_dispatch`, 05:37:41, success, **2,034 listings**). 09-02 produced two scheduled runs: 06:21 (run 47, success, left `data.json` byte-identical to my dispatch) and **09:31 (run 48, FAILURE — see O-27)** | `actions_list` 05:37 today; run 47/48 logs | **Per the standing correction I do NOT call today's absence — nothing has landed before 06:00 since 08-26 and any pre-11:00 reading is worthless as evidence of absence.** Fix unchanged: `repository_dispatch` from a Vercel cron; Vercel's scheduler has been exact all eight nights. Blocked on `GITHUB_DATA_TOKEN`. **No day lost: 08-27..09-03, all eight by my hand** | **HIGH — day 8** |
| O-27 | **RedSP is challenging GitHub Actions egress, and the curl fallback does NOT get through it. SECOND CONFIRMED INSTANCE TODAY** | run 34 log (08-28 13:19), quoted in `4fae319`; **NEW: run 48, 09-02 09:31 — identical HTML interstitial, `openresty/1.31.1.1`, 12.2KB, and BOTH node fetch and curl refused across 4 attempts** | **This is no longer a single observation.** Two scheduled runs have now died on it while every ~05:37 dispatch got through — but runs 45 (09:59) and 42 (11:32) also succeeded at similar hours, **so time-of-day does NOT explain it and I should not claim it does.** Neither failure cost a day: my dispatch had already captured. Durable fix needs Henrik | **HIGH — escalated, 2 instances** |
...
**No new experiment today — EIGHTH consecutive day, and no read-out was due
either (next 09-08).** The reason is unchanged from yesterday and it is still
the right one: my experiment DISCIPLINE, not my experiment supply, is the
binding constraint, and O-59 — the next candidate — stays blocked until
**09-25** precisely so nothing else ships into its metric. Today's work was
pipeline reporting, which touches no indexable surface and so cannot
contaminate it. Yesterday's fuller reasoning: It is no longer only that pipeline work crowds it out: **the
...
**THE RISK TO THE EXPERIMENT LEDGER IS ACUTE, DAY 4.** Perplexity has been out
since 08-31. **09-03 was not a run day (atlas runs Mon/Wed/Fri), so there is no
new evidence today — the last measurement attempt remains 09-02, 401 on 74 of
74 lookups.** The next real test is **Fri 09-04**. **Four pending read-outs
...
|---|---|---|---|
| AVM median absolute error | **15.82%** (in-sample, n=**2,034**), MAPE 21.95%. Gate run reproduced the committed file exactly apart from `computed_at`, reverted as churn (fifth day running). **The move 15.51 -> 15.82 is the BOOK changing (2,043 -> 2,034), not a model change — no AVM code was touched** | 2026-09-03 | `public/model-stats.json` |
| Live book | **2,034 listings** (was 2,033) | 2026-09-03 | `public/data.json`, feed commit `4d41943` 05:38:19 (**manual dispatch, run 49, 8th day running**) |
| Sitemap | **2,685 `<loc>`** (was 2,695), valid XML, 5 sampled property URLs all 200 (N8232, SP1213, N9604, N9791, N8744) | 2026-09-03 | `/sitemap.xml`, parsed |
| Frontier sitemap | **130 property URLs** (was 120, 117, 116, 118, 121, 122, 127, 134) (O-59) | 2026-09-03 | `/sitemap-frontier.xml`, parsed |
| Corpus version | site **v2026-09-03** (= today ✓, generated 05:38:44Z) · `avena-data` mirror at **v2026-09-02** (its normal lag) · HF unverified (401 without a token) | 2026-09-03 | site + mirror raw |
| **How to read the mirror correctly** | avena-data's `daily-snapshot.yml` runs **07:15 UTC**; I run at **~05:40 UTC**. So the mirror ALWAYS shows yesterday's version when I look. **Compare after 08:00 UTC, or the mirror against the site's PREVIOUS day. Do not re-open this as divergence.** **NINE consecutive correct predictions** | 2026-09-03 | avena-data raw `market/dataset.json` |
...
| **Real price moves by day** | 15 (08-14), 4, 1, 0, 15, 10, 10, 18 (08-21), 9, 0, 0, 3, 6, 5, 6 (08-28), 6, 0 (08-30), 1 (08-31), **7 (09-01)**, **12 (09-02, re-read and CONFIRMED)**, **16 (09-03, provisional)**. **Definition now fixed: a move is counted only when the previous snapshot is ≤2 days earlier.** On that definition 09-01 is **6**, not 7 | 2026-09-03 | `price_snapshots`, direct SQL diff |
...
| Snapshot rows by day | 2,041 (08-27), 2,047, 2,042, 2,044 (08-30), **2,044 (08-31 — UNION, true final book 2,042, O-74)**, 2,043 (09-01), 2,033 (09-02), **2,034 (09-03)** | 2026-09-03 | `price_snapshots` |
| Today's capture | feed 2,034 · snapshotted 2,034 · moves_detected 16 · route `delisted: 0` but **7 real tombstones by parse-feed** · **snapshot_superseded 0** · prior 09-02, age 1d, trusted · overlap **0.997** · errors null | 2026-09-03 | `/api/cron/pricing-history` |
| Delistings | **7 new tombstones today, ALL stamped `last_seen_date` 09-03 when last truly seen 09-02 — all 7 wrong.** Cumulative **114: 31 correct · 73 one day late · 10 stamped behind = 73% wrong** (was 62%, 51%) | 2026-09-03 | `sold_properties`, re-derived |
...
| Build health | **Run 49 (dispatch, 09-03 05:37) success**, book committed `4d41943`. **Run 48 (scheduled, 09-02 09:31) FAILED on the RedSP bot challenge — O-27, and it cost no data.** No open PRs. **One push to main today** (`07fbf93`); all four gates green before it. No red checks on any branch | 2026-09-03 | `actions_list` |
...
| **Cron logging coverage** | **64/64 scheduled crons write to `cron_logs`**. **0 hardcode their own status**, verified live 09-03: 19 agents ran, 15 `success` / 4 `error`, **0 false alarms**. `invoked_by` on real scheduled runs = **`vercel-cron-ua`** (User-Agent, not the header) | 2026-09-03 | live `cron_logs` |
...
| **Ingest write funnel (NEW)** | dvf-ingest 09-03 pre-fix: fetched 8,562 · inserted 6,716 · **apparent gap 1,846, TRUE loss 50.** Worst historical loss found: **08-27 Nice ~588 rows across ~12 failed chunks, reported as 5 error strings, logged `success`.** As of `07fbf93` the funnel reports `fetched -> deduped -> written (+ lost)` with `errors_total` uncapped | 2026-09-03 | `cron_logs` × 14 runs |
...
| Test coverage added by Odyssey | `test-open-dataset` 27 · `test-scribe` 22 · `test-cron-coverage` 99 · `test-integrity` 15 · `test-capture-integrity` 19 · **`test-chunked-write` 54 (NEW)** | 2026-09-03 | `07fbf93` |
| `causal_indicators` | **20 rows, ONE distinct `last_updated`: 2026-05-23 10:53:08** (O-54) | 2026-08-24 | queried directly |
| APCI macro input age | **103 days** (`as_of` 2026-05-23) — climbing daily until O-34/O-40 resolved | 2026-09-03 | `/api/v1/apci` |
...
|---|---|---|
| **BRANCH AWAITING APPROVAL: `odyssey/absorption-ledger-dates`** (`d182cd6`) — **DAY 18. THE WINDOW CLOSES TOMORROW.** | **83 of 114 delisting dates are wrong — 73% — re-derived today. 62% yesterday, 51% the day before: today's 7 departures were ALL stamped a day late.** The error grows by the daily delisting count, every day. **Plan B Release 1's data window closes 2026-09-04 — TOMORROW — and Release 1 quotes delistings BY DAY. If this is not approved today, Release 1 either slips or ships a delisting date series that is 73% wrong; I will not compute its slots from this table as it stands.** | **Three sentences: (1) parse-feed derives the real last-seen date from `price_snapshots` instead of stamping today, and `buildLedger` counts a delisting on the first observation day AFTER it — the two must land together. (2) `scripts/backfill-tombstone-dates.sql` corrects the historical rows; its read-only dry run moves each back exactly one day and touches nothing else. (3) Branch-only because it mutates an existing column on `sold_properties`, the one table here that cannot be rebuilt.** All four gates pass. **Re-run the dry run against today's 66 before applying, and note the O-74 caveat: a union day inflates the "correct" count in exactly this comparison.** |
| **THE CAPTURE DEPENDS ON ME BEING AWAKE — EIGHT mornings running** (O-61/O-27, day 7 of asking) | GitHub's scheduler has run this repo's nightlies 5–12h late or not at all on 08-27 through 09-03 — **both workflows every time, so it is repo-wide.** All eight days were saved by my hand dispatch at ~05:37. **No day has been lost.** **New today: the 09-02 09:31 scheduled run FAILED OUTRIGHT on RedSP's bot challenge (O-27, second confirmed instance) — so the late runs are not merely late, they can also come back empty.** The cost is real and not hypothetical: the 08-31 union day (O-74) exists because my early dispatch and GitHub's late run captured two different books into one day. | **One thing, unchanged since 08-29: `GITHUB_DATA_TOKEN` with `repo` scope in Vercel env.** Then I drive the feed from a Vercel cron via `repository_dispatch`, and Vercel's scheduler has been exactly on time on all eight of these nights while GitHub's was not. **Still the single highest-value two-minute action available to you.** Secondary: **ask RedSP to allow-list GitHub Actions egress for the feed URL.** |
| **THE PERPLEXITY BALANCE IS OUT — the citation engine has been dark since 08-31** (day 4) | Thursday 09-03 was NOT a run day, so there is no new evidence today; the last run day, Wednesday 09-02, all three atlas invocations failed 74 of 74 lookups on `HTTP 401 "You exceeded your current quota"`. **The `9171dce` guard held — no fabricated 0.00% published.** But the engine that scores the entire AI-citation thesis is dark, and **four pending experiment read-outs (09-08, 09-09) depend on it.** **Ignore `plab-run`'s `keys: {perplexity: true}` — that reports the env var is set, not that it has credit.** | **Top up the Perplexity balance, or tell me not to.** If you don't, those read-outs are recorded **UNMEASURABLE** rather than "no effect" — different findings, and conflating them would corrupt the ledger. **Since `908be3a` this failure logs as `error` in `cron_logs` with reason `raw_rows_absent_on_a_run_day`, verified live today — it can no longer hide.** |
| **"CRYPTOGRAPHIC VERIFICATION" IS PROMISED ON EIGHT SURFACES AND THE ZENODO HALF IS STILL NOT TRUE** (day 5) | I fixed the half I own: as of `14eae61` Avena genuinely fingerprints its daily batch, model snapshot, dataset manifest and methodology weights into a real Merkle root — verified unattended again on 09-01. **What is still false is the Zenodo half.** No code deposits a daily root, every root's `zenodo_url` is null, and these say otherwise in the present tense: **/verify**, **/stack**, **/proof**, **/apon-network**, **/eu-presidency**, **/papers/delphi**, **/methodology**, **/methodology/evolution**. Worse: **`src/lib/outreach.ts` puts "cryptographic integrity with Zenodo-anchored Merkle roots" into outbound pitch email to institutions.** | **Your call on the copy, and I need it more here than on SHAP because this one goes out in email.** **(a)** I change the Zenodo/RFC-3161 clause to state what is true, on all eight surfaces + outreach.ts — smallest possible edit, no layout change; **(b)** you give me `ZENODO_TOKEN` and I automate the deposit, making the claim true rather than smaller; **(c)** you write the replacement wording. **I have already corrected `llms.txt` in place.** I have not touched the pages or the email. |
...
| **THE ANTHROPIC API BALANCE IS EXHAUSTED — degrading seven jobs** (standing, day 12) | `predictions/generate`, `digest`, `generate-briefs`, `weekly-alpha` error on "credit balance is too low"; `delphi-run` and `plab-run` skip the Claude panelists (`models_scored: []`); `pulse` fails HTTP 500. `sync-regulatory-signals` fails classification for this reason, which is why `regulatory_signals` has ingested nothing since 08-04. | **A decision, not a task: top up or don't.** If you top up, `predictions/generate` starts publishing LLM-authored forecasts on `/track-record` — the class of surface that produced the `precursor-scan` fabrication, so **say so explicitly if you want that live**. If you don't, tell me and I'll make the affected routes report `skipped` with a stated reason instead of failing nightly. **The quieter harm: DELPHI and PLAB publish a "panel" consensus that is now, on some days, no models at all.** |
