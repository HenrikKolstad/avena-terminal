...
| **Search Console Generative AI report — DONE, data in repo** | **Henrik exported it 2026-08-14 (same morning it was asked); CSVs live in `docs/gsc-genai/*-2026-08-14.csv`.** Headline (3 months to 08-14): **228 impressions in Google AI features, 129 distinct URLs cited, weekly trend ~0-9 (May-Jun) → 20 → 40 → 53 (peak, w/c 07-27) → 41 → 23 (partial week) — roughly 10x growth since June.** THE structural finding: **/compare pages are 87% of all AI-feature impressions (198/228)**; top page `compare/torrevieja-alicante-vs-torremolinos-m-laga` (20). Guides 6, answers 2, statistics 0 — the compare FORMAT is what Google's AI cites, and those pages carry NO ledger data today. Audience: Spain 71, UK 35, Belgium 13, NL 9, NO/SE 8 each. Still UI-only/no API, so refreshes stay manual — re-export monthly, next ~2026-09-14, to serve as read-out data for the experiments in section 3. | Fable 2026-08-14. Candidate play (needs Henrik's go, not yet built): put the Nightly Quotable observation sentences on the /compare pages — the moat content onto the surface Google already cites. |
