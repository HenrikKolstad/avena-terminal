...
|---|---|---|---|
| 2026-09-07 | `5e70b6d` **`resilient-fetch` — a single transient blip zeroed a whole source for the day and reported it as the four words "fetch failed".** All six eu-stats adapters called `fetch` once, no retry, NO TIMEOUT. Now 15s/attempt, 3 attempts, per-source wall-clock budgets, `describeFetchError` unwrapping undici's `cause` | **PARTLY VERIFIED IN PRODUCTION THE SAME DAY, on the half that could refute me. Ran the deployed route at 06:12: `eurostat` **3,808** and `ecb_sdw` **537** — both EXACTLY unchanged, so the new per-source budgets do NOT starve the healthy sources, which was the real risk of this change. `ine_es` 4,480, total 8,825, `rows_lost` 0 everywhere. Whole run took **31.6s against a 265s global budget**, so the slices are generous rather than tight.** **The 5xx retry path DID execute in anger: ISTAT returns HTTP 500, which is retryable, so it was attempted 3 times with backoff and still reported honestly; BIS's 404 correctly failed FAST without retrying.** **STILL NOT VERIFIED, and I must not round this up: the NETWORK-ERROR retry (the actual INE failure mode) has never executed, because every upstream answered on the first attempt — a green hand-run proves the plumbing, not the retry. And `describeFetchError`'s unwrap has only ever met fixtures I built from undici's documented shape, never a captured production error.** **09-08 04:15 is the read-out: (a) `rows_upserted` ≥ 8,825 with NO hand re-run; (b) all loss counters 0; (c) eurostat 3,808 / ecb_sdw 537 still; (d) if INE fails again the error must NAME a cause and an attempt count, never a bare "fetch failed"** | **budget + 5xx-retry halves VERIFIED same-day; network-retry half awaits a natural failure** |
| 2026-09-07 | `5e70b6d` then `b21de95` **O-81, both surfaces — and I broke one of them in the first attempt.** `/defensibility`'s count is now read LIVE; `/api/openapi.json` no longer advertises three sources that hold no rows | **openapi VERIFIED LIVE: `source` enum is exactly `['eurostat','ecb_sdw','ine_es']` and the description names the dormant three with reasons.** **`/defensibility` VERIFIED LIVE for the row count (8,825) and REFUTED for the country count — it published "7 countries" against a true 28, because `.limit(20000)` silently hit PostgREST's 1,000-row cap. Fixed in `b21de95` by paging explicitly and returning null unless the scan COMPLETED.** **TOMORROW: confirm the page reads "8,825 observations, 28 countries" (the number will have grown — what matters is that the country figure is 28, not 7), and confirm it did NOT regress to dropping the parenthetical, which would mean my completeness check is now too strict and never fires** | **openapi VERIFIED; defensibility fixed on the second attempt, re-check 09-08** |
| 2026-09-06 | `3a55753` **INE Spain — READ OUT TODAY, AND IT SPLIT.** | **THREE OF FOUR CRITERIA PASSED ON THE SCHEDULED RUN; CRITERION (a) FAILED, FOR A NEW REASON.** **(b) 0 fabricated Q5-Q9 ✓. (c) `rows_lost` 4,480 → **0**, `write_chunks_failed` 9 → **0**, `rows_duplicate_excluded` 0, `rows_undecodable` 0 ✓ — the nightly 4,480-row bleed is gone. (d) eurostat + ecb_sdw still exactly **4,345**, so the new guard is NOT over-broad ✓.** **(a) FAILED: `ine_es` held 0 rows at 05:40, because the 04:15 run died on `INE 25171: fetch failed` — a NEW error, one step earlier than the one I fixed.** **Established transient, not a regression: INE answered 200 on 8/8 manual attempts (1.2–5.7s, 492KB), and `3a55753` never touched the fetch line.** **A hand re-run of the route then landed 4,480 rows: 20 real quarters 2021-Q1..2025-Q4, 0 fabricated, table now 8,825 across 28 countries and 3 sources.** **So the decoder fix is VERIFIED and the DELIVERY was the defect — which is what `5e70b6d` fixes** | **VERIFIED on (b)(c)(d); (a) met only after a manual re-run → the gap became today's work** |
| 2026-09-06 | `47fc0b4` **THE CAPTURE BACKSTOP — the scheduled half AND the dispatch-decision half, both read out today** | **BOTH VERIFIED. (1) SCHEDULED, UNATTENDED: runs 2 and 3 fired on `event: schedule` (09-06 11:28 and 12:31), both green, both `action=no_op reason=snapshot_present degraded=false`, embedded decision-rule test green in CI both times, all three production signal readers correct with no `indeterminate` fallback. (2) THE DISPATCH DECISION PATH — which this file said needed 'a genuine nightly failure' and could therefore wait a fortnight — was exercised TODAY at 06:07 with `dry_run: true`, because before the nightly lands the day is GENUINELY uncaptured, so the dispatch branch is reachable on real signals at zero risk: `snapshot: none · in-flight: no · book: generated_date 2026-09-06 (today is 2026-09-07)` → `action=dispatch reason=snapshot_absent degraded=false`, and crucially `dispatched=false` because dry-run held. 37/37 tests passed in CI on the same run.** **I read the guard before firing rather than trusting yesterday's dry-run: the POST sits strictly inside the `else` of `if (DRY_RUN)`, so it is unreachable by construction — and that mattered, because yesterday's dry-run took the NO-OP branch and therefore proved nothing about dry-run on the dispatch branch.** **STILL NEVER EXECUTED: `dispatchFeedRefresh`'s actual POST. Only a real failure proves the token has `actions: write`. If it is malformed the backstop would decide correctly and then fail — loudly (it exits 1), but fail.** **UNWELCOME FACT, recorded not buried: scheduled for 07:00 + 09:00, the runs landed 11:28 and 12:31 — 4h28m and 3h31m late. Queue lag is per-workflow and does NOT transfer from the feed's 65–93 min** | **VERIFIED on no-op AND on the dispatch DECISION; the POST itself still unproven** |
| 2026-09-04 | `141bf2e` **dvf-ingest's orphan exclusion** and **`fetchCommuneYear` throwing on a dead upstream** | Orphan exclusion **VERIFIED 09-05 on an affected commune** (Le Lavandou 2023, funnel identity exact) → CLOSED. The throwing path's **negative is bounded** (28/28 commune-years return 200, so it cannot false-alarm); **the positive still awaits a natural upstream outage, and the tell is `dvf-ingest` logging `error` with `DVF fetch failed: HTTP …` instead of `fetched: 0` + `success`** | **NEGATIVE BOUNDED; positive awaits a natural outage** |
...
| O-83 | **NEW, AND THE BIGGEST CREDIBILITY ITEM I FOUND TODAY — the property count "1,881" is hardcoded on ~20 public and AI-facing surfaces while the real book is 2,034.** It is wrong by 153 listings (7.5%) and grows more wrong every night the feed adds a unit | **CONFIRMED SERVED, not just present in source: the live homepage returns "1,881" FOUR times, twice inside the JSON-LD `Dataset` description ("Depth: 1,881 scored Spanish new-builds" and "1,881 new-build properties across Costa Blanca…") — i.e. the exact payload AI crawlers ingest.** `python3` count of `public/data.json` = **2,034**; `grep -rn "1,881"` over `src/app` returns 20 files including **`structured-data.ts` ×2 (the site's JSON-LD)**, `api/v1/datasets`, `api/mcp-tools`, `api/model/training-data`, `api/v1/prediction-oracle`, `api/synthetic`, `opengraph-image.tsx`, `terminal/TerminalChat.tsx`, `mcp-server/page.tsx`, `tiktok/*`, `challenge/score-2026` | **It UNDERSTATES rather than overstates, which is why it is not an emergency — but it is a false published number sitting on exactly the JSON-LD and MCP surfaces the AI-citation channel reads, so the models are being taught a wrong figure. Fence 2 exception (a) explicitly permits correcting it.** **DELIBERATELY NOT DONE TODAY: the right fix is ONE derived constant, not 20 hardcoded edits that go stale again next week, and it touches rendered buyer-facing copy in several files. I had already shipped a pipeline change and two credibility edits; doing this badly would be worse than doing it tomorrow properly.** **Prescription: export the count from `public/data.json` once and reference it; check each site for prose that must agree grammatically ("the 1,881-property corpus"). CLAUDE.md's own "1,881" needs the same correction** | **HIGH — credibility, ~20 surfaces, act 09-08** |
| O-84 | **NEW — the bare-`fetch` shape is fixed in `eu-stats-feeds.ts` and NOT swept for elsewhere. SIZED TODAY: 83 `await fetch(` call sites carry no `signal`/timeout, of which ~30 are SERVER-SIDE and cron-reachable** | swept 09-07. The ones that matter: **`api/cron/`** regime-check, auto-post, developer-monitor, github-snapshot ×3, pulse, deal-alerts, research-lab, social-delphi ×2, sync-feeds, mentat · **`src/lib/`** crawler-submit, prometheus, regulatory-intent, citation-agent, search-console ×2, integrity, data ×2, **data-sources/{osm,eurostat,dvf,catastro}** · plus check-price-drops, v1/archive/verify, auto-post ×3, analyze ×2. The other ~53 are browser `.tsx` forms, where the browser has its own timeout and a hang is a UX bug, not data loss | **`src/lib/indexnow.ts` and `src/lib/webhooks.ts` already use `AbortSignal.timeout`, so the convention exists and is applied unevenly. `data-sources/*` and `sync-feeds` are the highest value — they are ingest paths where a hang costs a day of a source, exactly like INE did today.** Not swept today because the sweep is worth doing once, carefully, and today's budget went to making the fix correct rather than wide. **`fetchWithRetry` is now available to all of them** | medium |
...
**No new experiment today — TWELFTH consecutive day. No read-out was due today, and the next two are TOMORROW, 09-08.** Both of today's changes are
pipeline and ingest work: neither touches an indexable surface, so neither can
contaminate a pending read-out. **Weekly search scan NOT due — done 09-02, next
09-09. Running it early to have something to report would be the manufactured
work the brief forbids.** The reasoning from 09-04 is unchanged and still right:
**my experiment DISCIPLINE, not my experiment supply, is the binding
constraint.** Two read-outs came due on 09-02 and BOTH failed on design rather
than on data — one had no pre-change baseline, the other had six co-shipped
changes inside its window. Starting another before fixing that would produce
another unreadable result. O-59 (narrowing the frontier window) remains the next
candidate and stays blocked until **09-25**, by which point nothing else may
ship into its metric.

**THE RISK TO THE EXPERIMENT LEDGER IS NOW REALISED, DAY 8 — AND THE READ-OUTS ARE TOMORROW.** Perplexity has been out
since 08-31. **TODAY, 09-07, WAS A RUN DAY — I had the weekday wrong (see
corrections). atlas ran at 03:00, 03:10 and 03:20 and failed ALL THREE on
`Perplexity HTTP 401 "You exceeded your current quota"`, `lookups_failed: 74`,
`lookups_measured: 0`, `persisted_this_run: 0`.** `cassandra` correctly logged
`error` with `2026-09-07: raw_rows_absent_on_a_run_day` while still reporting
09-06 as `no_run_scheduled` — the `908be3a` distinction working on the exact
discriminating pair. **The engine has measured nothing since 08-28 — TEN days.**
**The two read-outs due TOMORROW (09-08) are measured by this engine, and today's
run is direct evidence they will have no after-data.** The next run day after
that is Wednesday 09-09, which carries three more. Unless the balance is topped up TODAY they are
recorded **UNMEASURABLE**, never "no detectable effect". Conflating them would
be the exact failure the 08-25 correction is about.
...
| **2026-09-07** | **Release 1 proposed fire, 08:00 CET with Monday Pulse — THAT IS TODAY** | **RECOMMENDATION UNCHANGED AND NOW DUE: SLIP IT. Re-derived again this morning — 130 tombstones, 31 correct, 89 one day late, 10 stamped behind = 99 wrong, 76.2%, identical to yesterday (no departures overnight, so it did not grow).** The gate has been unmet since the window closed on 09-04 and `odyssey/absorption-ledger-dates` is unapproved on **day 21**. Two options for Henrik: **(a) slip the whole release until `odyssey/absorption-ledger-dates` is approved and the backfill has run** — my recommendation; **(b) fire the PRICE-MOVE half alone, which is fully sound**, and hold the absorption half for Release 2. **What must not happen is firing the delisting-by-day series as drafted.** |
...
|---|---|---|---|
| AVM median absolute error | **15.76%** (in-sample, n=**2,034**), MAPE 21.87%. **Read AFTER tonight's feed run, not before it — the whole point of the note below.** My 05:45 gate run reproduced the committed file exactly (15.71% / 2,026) against the 09-05 book; the 06:27 nightly then regenerated it against the 09-06 book | 2026-09-06 (post-run) | `public/model-stats.json` |
| **RECURRENCE, not a new mistake — read this before recording the AVM figure again** | Yesterday I recorded 15.76% / n=2,037 and corrected the day before that from 15.89% / n=2,033. **All three were right when taken and stale within hours**, because the nightly regenerates `model-stats.json` and I measure at ~05:45, before the day's feed has run. **The number in this table is therefore always the PREVIOUS book's, and that is now stated rather than re-discovered: it is 15.71% / 2,026, from the 09-05 book.** The movement 15.89 → 15.76 → 15.71 is entirely the BOOK; no AVM code has been touched since 08-07 | 2026-09-06 | `git log public/model-stats.json` |
| Live book | **2,034 listings** (was 2,026) — **8 arrivals, 0 departures**, confirmed by direct diff not by the route's field | 2026-09-06 (post-run) | `public/data.json`, feed run 58 |
| Sitemap | **2,686 `<loc>`** post-run (2,678 pre-run — **the +8 is exactly the 8 arrivals, reconciled**), valid XML, **0 duplicates**, 5 sampled property URLs all 200 (N7747, N9399, N9560, N9225, SP1019). Composition pre-run: **2,026 /property (exactly the book), 436 /compare, 99 /towns, 35 /answers, 11 /costas, 3 /no**. All four sitemaps declared in robots.txt return 200 | 2026-09-06 | `/sitemap.xml`, parsed |
| Corpus version | repo **v2026-09-06** (committed 06:28:21) · site **v2026-09-05** at 06:30 — **the Vercel deploy of the corpus commit was two minutes old and still in flight; this is deploy lag, NOT divergence** · mirror **v2026-09-05**, correct because it runs 07:15 UTC (thirteenth consecutive correct prediction) | 2026-09-06 | site + repo + mirror raw |
...
| **`eu_official_stats` — the first three-source day in the table's history** | **8,825 rows, 28 distinct countries, THREE sources: `eurostat` 3,808 · `ecb_sdw` 537 · `ine_es` 4,480 (2021-Q1..2025-Q4, 20 real quarters, 0 fabricated).** `istat`, `cbs` and `bis` still hold ZERO and always have (O-82). **Landed 09-07 only after a manual re-run — the 04:15 scheduled run died on a transient `fetch failed`; `5e70b6d` is the fix and 09-08 is its read-out** | 2026-09-07 | direct SQL |
| **Ingest write funnel — eu-stats-ingest** | **POST-FIX (09-07 05:40 re-run): `rows_upserted` 8,825 · `rows_lost` 0 · `write_chunks_failed` 0 · `rows_duplicate_excluded` 0 · `rows_undecodable` 0 · errors 2 (istat HTTP 500, bis HTTP 404 — both upstream, O-82).** Pre-fix it was `rows_upserted 4,345 · rows_lost 4,480 · chunks_failed 9`, identical on 09-04/05/06 — **it lost more than it wrote, every night, for months** | 2026-09-07 | `cron_logs` |
...
| **Real price moves by day** | 15 (08-14), 4, 1, 0, 15, 10, 10, 18 (08-21), 9, 0, 0, 3, 6, 5, 6 (08-28), 6, 0 (08-30), 1 (08-31), 6 (09-01), 12, 16, 19, 19 (09-05), 1 (09-06), **0 (09-07)**. **09-07 verified by direct SQL diff AND by the route's `moves_detected` — they agree, and the book diff explains it: one listing changed, on non-price fields only.** **Definition: a move counts only when the previous snapshot is ≤2 days earlier** | 2026-09-07 | `price_snapshots`, direct SQL diff |
| Snapshot rows by day | 2,047 (08-28), 2,042, 2,044, **2,044 (08-31 — UNION, true final book 2,042, O-74)**, 2,043 (09-01), 2,033, 2,034, **2,037 (09-04 — two writes, clean superset)**, 2,026 (09-05), 2,034 (09-06, ONE write), **2,034 (09-07, ONE write at 06:36:00)** | 2026-09-07 | `price_snapshots` |
| **09-06's capture — FINAL and clean, including the second run I had not seen when I wrote yesterday's state** | `price_snapshots` 09-06: **2,034 rows, 2,034 refs, a SINGLE `created_at` at 06:27:44.** **Feed run 59 ALSO ran on 09-06, at 09:20 — a second run I did not know about yesterday — and it wrote NOTHING: it drew a byte-identical book, so `data.json` was not re-committed and no second snapshot landed. NO UNION on 09-06.** Confirmed by direct SQL (`distinct created_at` = 1), not by the route's field | 2026-09-07 | `price_snapshots`, `actions_list` |
| **TODAY'S CAPTURE (09-07) — LANDED AND CLEAN. FOURTEEN DAYS, NONE LOST** | Scheduled run landed **06:36:05 — an 86 min queue lag off the 05:10 slot, squarely in the 65–93 min band**. `price_snapshots` 09-07: **2,034 rows, 2,034 refs, a SINGLE `created_at` at 06:36:00 — no union.** Route (post-deploy): feed 2,034 · snapshotted 2,034 · `price_moves` 0 · `moves_baseline_refs` **2,034** (a baseline existed, the diff was not blind) · delisted 0 · `snapshot_superseded` 0 · prior 09-06, age 1d, **trusted** · overlap **1.000** · errors null. **INDEPENDENTLY RE-DERIVED by direct SQL rather than trusting the route's fields, and they agree exactly: 0 price moves, 0 arrivals, 0 departures.** **A 0/0/0 day with an identical book size is the kind of number I have twice built a false story around, so I decomposed it instead of accepting it: diffing the two committed books shows exactly ONE listing changed, on four non-price fields (`lat`, `lng`, `c`, `dev_ref`). A genuinely static day, NOT a re-banked stale book — which is what the byte-identical case would have looked like (no re-commit at all).** **A SECOND feed run followed later on 09-07 (`d7049ed`) and correctly wrote NOTHING: it drew a byte-identical book, so `data.json` was not re-committed (git log shows `0af5557` as the day's only book commit) and no second snapshot landed — only `feed-meta.json` was stamped, which is the documented mechanism. `price_snapshots` for 09-07 still shows ONE `created_at`. NO UNION. Same benign pattern as run 59 on 09-06; do not re-investigate that commit.** **The backstop was not needed: the nightly landed at 06:36 and its slots land ~11:00+. `dispatchFeedRefresh`'s POST therefore still has never executed** | 2026-09-07 | `cron_logs`, `price_snapshots`, direct book diff |
...
| **09-05's capture, now FINAL and confirmed clean** | The day ran THREE feed workflows — run 55 (06:15:40, FAILED on O-27), run 56 (06:58:02, my dispatch, green), run 57 (08:59:12, scheduled, green). **Despite three runs, `price_snapshots` for 09-05 holds 2,026 rows with a SINGLE `created_at` (06:58:26). Run 57 drew a byte-identical book, so `data.json` was not re-committed and no second snapshot was written. NO UNION.** **Useful mechanism confirmed here and relied on by the new backstop: `feed-meta.json` IS committed on every successful run (its `generated_at` always changes) even when the book is identical — which is why "the committed feed-meta's generated_date" is a signal for "the feed ran today", not merely "the book changed today"** | 2026-09-06 | `actions_list` + `price_snapshots` + `git log` |
| Delistings | **130 tombstones: 31 correct · 89 one day late · 10 stamped behind = 99 wrong (76.2%)** — re-derived 09-07, IDENTICAL to 09-06. **Flat only because there were no departures overnight; it resumes growing on the next departure day.** This is the unmet gate under Plan B Release 1, whose proposed fire date was today | 2026-09-07 | `sold_properties` × `price_snapshots` |
| **NIGHTLY RELIABILITY — the 05:10 slot's queue lag** | 06:43 (09-01), 06:21, 06:24, 06:29, 06:15, **06:27 (09-06)** — a consistent 65–93 min lag over six nights. **09-07 not yet measurable at 06:00 UTC; do not read it before ~11:00 (standing rule).** **SEPARATE AND MUCH WORSE: the capture-backstop workflow is scheduled 07:00 + 09:00 and its first scheduled runs landed 11:28 and 12:31 — 4h28m and 3h31m. Queue lag is per-workflow and does NOT transfer between them** | 2026-09-07 | `actions_list` |
| Build health | **No red checks on any branch; no open PRs.** **One push to main today (`5e70b6d`), all four gates green before it; a follow-up push corrects a truncated read I shipped in it (see corrections).** Reds in `cron_logs` over 24h are all known: prometheus ×4 + pulse + generate-briefs + predictions-generate + causal-update (Anthropic balance), atlas ×3 (Perplexity), auto-post ×3 (O-53), counterpart-discover (O-41), eu-stats-ingest ×2 (04:15 transient + my re-run, correctly red on istat/bis upstream), cassandra (correct — a run day with no measurement) | 2026-09-07 | `cron_logs`, `actions_list` |
| Test coverage added by Odyssey | `test-open-dataset` 27 · `test-scribe` 22 · `test-cron-coverage` 99 · `test-integrity` 15 · `test-capture-integrity` 19 · `test-chunked-write` 71 · `test-prometheus-reporting` 25 · `test-capture-backstop` 37 · `test-eu-stats-keys` 40 · **`test-resilient-fetch` 38 (new)** | 2026-09-07 | `5e70b6d` |
...
| **CITATION ENGINE — DARK SINCE 2026-08-31, DAY 8; LAST REAL MEASUREMENT 08-28, TEN DAYS** | **09-07 IS A MONDAY AND WAS A RUN DAY** (I had this wrong — see corrections). atlas ran 03:00/03:10/03:20, all three `measurement_failed` on `Perplexity HTTP 401 "You exceeded your current quota"`, `bank_size 74`, `lookups_failed 74`, `lookups_measured 0`, `persisted_this_run 0`. **`cassandra` logged `error` with `2026-09-07: raw_rows_absent_on_a_run_day` while keeping 09-06 as `no_run_scheduled` — the `908be3a` distinction working on the discriminating pair.** **The `9171dce` guard is holding: no fabricated 0.00% published.** **Note `plab-run` reports `keys: {perplexity: true}` — that means the env var is SET, not that it has credit** | 2026-09-07 | `cron_logs` |
...
| Cron success rates (worst) | **7 agents red in the last 24h, all known and all genuine:** `prometheus` ×6 (Anthropic balance, now naming itself) · `auto-post` ×3 (O-53) · `pulse` (Anthropic) · `generate-briefs` (Anthropic) · `causal-update` (Anthropic, `debate_null`) · `predictions-generate` (Anthropic) · `counterpart-discover` (O-41, NULL summary) · `eu-stats-ingest` (fixed today by `3a55753`; should go green 09-07). **`cassandra` is GREEN today and correctly so — Sunday is not a run day** | 2026-09-06 | `cron_logs` |
...
|---|---|---|
| **BRANCH AWAITING APPROVAL: `odyssey/absorption-ledger-dates`** (`d182cd6`) — **DAY 21, AND THE RELEASE 1 FIRE DATE IS TODAY** | **99 of 130 delisting dates are wrong — 76.2% — re-derived again this morning (31 correct, 89 one day late, 10 stamped behind). Flat since yesterday only because there were no departures overnight; it resumes growing on the next departure day.** **Release 1 was pencilled to fire TODAY at 08:00 CET and its delisting-by-day series would be three-quarters wrong.** **Plan B Release 1 quotes delistings BY DAY. I have NOT computed its slots and will not until this lands. My recommendation is unchanged: the 09-07 fire date slips rather than the accuracy — the price-move half of Release 1 is sound and could fire alone.** | **Three sentences: (1) parse-feed derives the real last-seen date from `price_snapshots` instead of stamping today, and `buildLedger` counts a delisting on the first observation day AFTER it — the two must land together. (2) `scripts/backfill-tombstone-dates.sql` corrects the historical rows; its read-only dry run moves each back exactly one day and touches nothing else. (3) Branch-only because it mutates an existing column on `sold_properties`, the one table here that cannot be rebuilt.** All four gates pass. **Re-run the dry run against today's 130 before applying, and note the O-74 caveat: a union day inflates the "correct" count in exactly this comparison.** |
| **BRANCH AWAITING APPROVAL: `odyssey/transactions-dedupe`** (`2fb0c3d`) — day 3 | **`/engine` currently tells every reader — buyers, institutions, and every AI that crawls it — that Avena holds ~396,000-503,000 "verified" registered transactions. The real number is 55,888.** The table has no unique key and dvf-ingest re-inserts the same French communes every rotation, so it is 88.9% duplicates and grows ~3,000 rows a night. **Same class as the "±3% RMSE with no backtest" episode, and it is on the surface that sells the moat.** | **Three sentences: (1) the migration collapses each `(avn_prop_id, transacted_at)` group to its earliest row and adds the unique index that makes the re-insert impossible; (2) the route's `.insert()` becomes an `.upsert()` on that key — the two MUST land together or every write fails; (3) branch-only because it deletes 447,546 rows from a table rebuildable only by re-crawling data.gouv.fr.** **Safety is measured: 0 duplicate groups disagree on price, 0 null keys, so nothing is discarded.** Run `npx tsx scripts/dedupe-transactions-dryrun.ts` and the three SQL checks it prints before applying. All four gates green. |
| **THE PERPLEXITY BALANCE IS OUT — TWO EXPERIMENT READ-OUTS COME DUE TOMORROW AND WILL BE UNMEASURABLE** (day 8) | The last real measurement was **08-28 — ten days ago**. **TODAY (Monday 09-07) WAS a run day and atlas failed all three attempts on `HTTP 401 "You exceeded your current quota"`: 74 questions queried, 74 failed, 0 measured.** **The two read-outs pre-registered for TOMORROW, 09-08, are measured by this engine. Today's run is direct evidence they will have no after-data.** **The `9171dce` guard is holding — no fabricated 0.00% has been published.** | **Top up the Perplexity balance TODAY, or tell me not to — this is the one blocker with a deadline attached.** If you don't, tomorrow's two read-outs are recorded **UNMEASURABLE** rather than "no effect" — different findings, and conflating them would corrupt the ledger. **Three more come due 09-09.** **Ignore `plab-run`'s `keys: {perplexity: true}`: that reports the env var is set, not that it has credit.** |
...
| **TWO "CLAIMED CAPABILITY, NO CALLER FOUND" ITEMS LEFT — I closed the third myself today** (O-70 day 7, O-58 day 12; **O-81 CLOSED by `5e70b6d`**) | **O-70:** `/about/methodology` lists **INE**, **Registradores de España**, **Idealista / Fotocasa** and **Banco de España** as Avena data sources; I could find no ingest path for Registradores, Idealista or Fotocasa. **(Note: the INE half of this improves tonight — `3a55753` lands ~4,480 INE rows, the first this project has ever held.)** **O-58:** "SHAP explainability" on `/methodology`, `/avm`, `/institutional`, `/standards/apip`, `/products/csrd-disclosure`, where the code computes hand-set rule weights. **O-81 (new):** `/api/openapi.json` advertises six sources over `eu_official_stats`; three of them (ISTAT, CBS, BIS) have never produced a row. | **Two questions. (1) Do you have a data agreement with Registradores/Idealista that I simply cannot see in this repo?** If yes, O-70 closes as my blind spot. If no, it is the same smallest-possible-edit decision as Zenodo. **(2) SHAP: (a) I change it to "rule-based feature attributions", or (b) you want real SHAP and I scope the AVM work.** **O-81 IS DONE — I corrected both surfaces myself today, as I said I would: `/defensibility` now derives its count live and `/api/openapi.json` no longer advertises three sources that hold no rows.** **And the INE half of O-70 is now genuinely TRUE — `ine_es` holds 4,480 rows as of today, the first this project has ever had — so the remaining question is only about Registradores, Idealista and Fotocasa.** |
