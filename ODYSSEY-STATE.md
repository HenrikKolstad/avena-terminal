...
|---|---|---|---|
| 2026-09-12 | `9f420e3` **`snapshot_superseded` conflated two OPPOSITE conditions and I had been reading it as one.** On 09-11 the route reported `2` at 09:35 (refs banked 55s earlier by parse-feed from a NEWER book — this run was the stale party, nothing had left) and `2` again at 14:30 (refs banked 06:35 and genuinely gone). Same field, disjoint refs, opposite meanings. Now split by the stored row's `created_at` against the book's generation INSTANT (new `getFeedGeneratedAt()`): `snapshot_superseded_stale`, `snapshot_ahead_of_this_book`, `snapshot_superseded_unclassified`. Report-only; the original total is untouched so the logged series stays comparable | **TWO halves, and the second is the one that can fail.** (a) CONTENT: on the next multi-write day the three new fields must appear in `cron_logs.output_summary` and must SUM to `snapshot_superseded` exactly (the test asserts it in-process; production is the real check). (b) TRACKING — **the discriminating failure is the fields being ABSENT, which means the deploy never landed and must NOT be read as three zeros.** Their presence with all-zero values on a single-write day is the guard working, not a regression. A `snapshot_ahead_of_this_book > 0` is the interesting case: it means that run held a stale book | **HALF (b) VERIFIED IN PRODUCTION THE SAME DAY, 07:06 — and it was the half that could fail.** The 09-12 capture returned all three fields PRESENT and all three `0`, with `snapshot_superseded: 0` beside them, on a single-write day. **Their presence is the deploy confirmed; absent would have meant the push never landed, which is exactly the reading the criterion was written to prevent.** `snapshot_superseded_unclassified: 0` also proves the book stamp was readable — an unreadable `generated_at` would have parked every candidate there. **(a) the SUM assertion is still UNEXERCISED: it needs a day with a non-zero total, and 09-12 had none. Re-registers for the next multi-write day.** | **(b) verified 09-12 · (a) pending** |
| 2026-09-12 | `b6979ae` **the O-87 cross-check: `property_pricing_history` and `price_snapshots` AGREE.** 08-10 onward: 273 snapshot-derived moves, 260 event rows, **260/260 matching on ref AND day, zero orphan events**; the 13 unmatched moves are all 08-11, the day before the event log began. Comment-only correction in `deltas.ts` — and a SECOND copy of the expired "contains ZERO price-move events" claim, 100 lines down, corrected with it | Nothing renders differently, so there is no production tell — which is why this gets a re-measurement instead of a verification: **re-run the same query on 2026-09-19 and confirm the agreement HOLDS as both writers keep running.** A divergence appearing later is the finding that would matter | **re-measure 2026-09-19** |
| 2026-09-11 | `1a1832d` **Two PUBLIC agent-status endpoints published `status: "active"` as a string literal for agents that had been dead for 12 and 26 consecutive runs.** `/api/citation-agent/status` and `/api/prometheus/status`, both advertised in `/api/index` for AI agents. Also: both `cadence` strings were false; eight reads sat behind `catch { /* ignore */ }` defaulting to 0/[]; `delta_vs_yesterday` subtracted two of those defaults so ONE failed read could publish a fabricated delta. Status now derives from `cron_logs`, cadence from `vercel.json`, unread values are null beside a recorded error | **VERIFIED IN PRODUCTION THE SAME DAY, by parsing both served payloads.** citation: `status:"failing"`, `consecutive_failures:12`, cadence `Mon/Wed/Fri 03:00, 03:10, 03:20 UTC (3 attempts)`, `citation_rate:null` with `rate_basis:"no measurement on this date — not a zero rate"`, `measurement_age_days:14`. prometheus: `status:"failing"`, `consecutive_failures:26`, cadence naming all four daily fires. **TOMORROW IS THE TRACKING HALF, AND IT IS THE HALF THAT CAN STILL FAIL — exactly the O-83 / `/engine` test one surface over.** (a) `consecutive_failures` MUST MOVE: prometheus 26 → ~30 by tomorrow (4 fires/day), atlas 12 → 15 on Mon 09-14 (next run day). (b) `measurement_age_days` MUST increment 14 → 15. (c) `observed_at` must be within minutes of my fetch, not a cached yesterday. **A FROZEN 26, or a frozen 14, means I shipped a literal wearing a function's clothes.** (d) **The other direction is equally diagnostic: if Henrik tops up either balance, `status` MUST flip to `active` without my touching the file.** (e) `errors[]` must stay `[]` while the reads are healthy — a non-empty errors[] with a null count is the guard working, NOT a regression | **VERIFIED IN PRODUCTION 2026-09-12, on the half I said could still fail.** Prometheus `consecutive_failures` **26 → 30**, which is exactly 4 fires/day over one day — the predicted number, not merely a different one. `measurement_age_days` **14 → 15**. Atlas `consecutive_failures` stayed **12**, which is CORRECT: 09-12 is a Saturday and Atlas runs Mon/Wed/Fri, so a move there would have been the bug. `observed_at` fresh within seconds of the fetch (first fetch served a `PRERENDER` from 09-11T09:36 and my request regenerated it — the ISR caveat from `9053781`, working as documented), `errors: []`, both `status: "failing"` with the live 401/400 quoted. **A frozen 26 or a frozen 14 was the discriminating failure and neither occurred. → CLOSED** |
...
|---|---|---|---|---|
| O-92 | **NEW, AND IT IS ABOUT MY OWN METHOD — I declare each day's capture "clean and quotable" three hours before the day is capable of being judged, and yesterday that published a false claim in my own state file.** I recorded 09-11 as **"a SINGLE write, clean, NOT a union day, its listing count IS quotable"** at 06:40. The day ended as a **union of THREE books**: 2,033 refs at 06:35:24, +2 (N8967, N9998) at 09:34:40, and by 14:30 two further refs (N8972, SP0664) had left. Stored **2,035 against a final book of 2,033** | `price_snapshots` `created_at` batches + `cron_logs` 09-11 ×4, re-derived 09-12 | **The defect is structural, not a slip: I verify at ~05:40–06:40 UTC and the second feed refresh lands ~09:35, so EVERY morning verdict is issued on an open day. It has been right most days by luck of there being one refresh.** The rule that follows, and it is now in VERIFY TODAY as a standing item: **a day's union status can only be judged the NEXT morning.** Today's brief states 09-11 correctly and 09-12's own status is deliberately left pending. **Not fixed in code today because the fix is not code — it is that my "quotable" column must lag by one day.** The durable version is a small derivation over `created_at` batches + the day's final `feed` size, which would replace the hand-kept union-day list (08-31, 09-09, **09-11**) with a query; that is the next candidate for a quiet day | **HIGH — it is a credibility defect in the provenance record Release 1 depends on** |
| O-91 | **NEW — a run holding a STALE book re-snapshots the whole day, so it can overwrite newer same-day prices with older ones.** The 09-11 09:35 run held the 06:35 book while parse-feed had already banked the 09:34 book, and it wrote 2,033 rows from the older book over the newer ones under the same `(ref, date)` key | `cron_logs` 09-11 09:35:13 — `snapshot_ahead_of_this_book` would have been 2 (measured after the fact); `snapshotted: 2033` | **No harm occurred on 09-11: the 14:30 run re-wrote from the newer book, so the day's final stored prices are the right ones. That is luck, not design — had 09:35 been the last run of the day, 09-11 would hold morning prices under a day whose book changed at 09:34.** Not fixed today for a stated reason: the repair (skip the snapshot write when a newer book is already banked) is a change to cron WRITE logic, and a buggy skip costs a whole day of capture — the most expensive failure available. **`9f420e3` makes the condition visible first; the repair waits for evidence that it has actually cost something, and then goes to a branch.** The evidence to look for: any day whose LAST write of the day reports `snapshot_ahead_of_this_book > 0` | medium-high |
| O-87 | **NEW, AND IT CORRECTS A LOAD-BEARING PROJECT FACT — `property_pricing_history` DOES hold move events now, and both CLAUDE.md and the code said it never had.** 394,000 dead `listed` rows (2026-04-29 → **2026-08-05**), plus **167 `increased` (08-12 → 09-08)** and **40 `reduced` (08-14 → 09-08)** | direct SQL 09-09, `group by status` with min/max `recorded_at` | **The 2026-08-08 audit ("not one 'reduced'/'increased' exists") was TRUE THEN and has been FALSE FOR FOUR WEEKS: the cron fix merged 08-08 began writing real events on 08-12. I have been carrying a stale fact as a live one.** Corrected in `src/lib/deltas.ts` today (`70e2d85`) — **the conclusion is unchanged (price_snapshots is ground truth because it is COMPLETE and per-ref-per-day) but it now rests on the true reason instead of an expired one.** **CLAUDE.md still carries the old wording and should be corrected — but that is Henrik's file, so it goes to NEEDS HENRIK rather than being edited unilaterally.** **THE OPEN QUESTION IS ANSWERED, 2026-09-12: THEY AGREE EXACTLY.** Over 08-10 onward, `price_snapshots` yields **273** moves (prior snapshot ≤2 days) and the event log holds **260** `reduced`/`increased` rows; **260 of 260 match a snapshot move on the same ref AND the same day, with zero orphan events**, and the **13** unmatched moves are **all dated 08-11 — the day before the event log started writing**. So the event log is a strict subset bounded by its own start date, and the two independent derivations do not disagree anywhere. Recorded in `deltas.ts` (`b6979ae`) with the numbers. **The conclusion still does not change: this is a reason to trust the log's CONTENT, not its COMPLETENESS — it begins 08-12, carries 394k dead `listed` rows, and is written by a second writer on its own schedule, so read paths stay on `price_snapshots`.** Re-measure 09-19 to confirm the agreement holds. **What remains open is only the CLAUDE.md wording, which is Henrik's file → NEEDS HENRIK** | **low — answered; only the CLAUDE.md line is left** |
| O-88 | **AwarioBot remains the largest consumer of the origin, but MY OWN FIGURE WAS WRONG AND I CORRECTED IT BY RE-DERIVING.** **Re-derived 09-12: 29,461 hits / 2,276 paths = 12.9 per path** over 7 days, against Googlebot **9,812 / 4,925**. Now **3.0x Googlebot**. Trajectory across three reads: 30,016 (09-09) → 26,725 (09-11) → 29,461 (09-12), so it oscillates rather than trends — **and its path count is EXACTLY 2,276 for the second consecutive day, which is the O-14 frozen-URL-set pattern returning after one day of movement.** | `crawler_hits`, 7-day window, re-derived 09-12 | **This is O-14 escalating, not a new finding, and its path count DID move this time (1,988 → 2,276), roughly tracking the book — so it is no longer "re-fetching a frozen URL set". It is fetching the right set ~13x more often than anything could need.** **DECISION REVERSED TODAY, AND THIS IS THE IMPORTANT PART. I came to this item intending to ship the `Disallow` — my own note called it "the cleanest small change available" and designated it for the next quiet day. Reading `robots.ts` first stopped me: the file records that AwarioBot was added to `BULK_TRAINING` on 2026-08-12 with "Henrik's explicit yes same day", and the recorded rationale is "Mention-monitoring value survives at any crawl volume; the contact form and resized images can never be a mention." So Henrik has already ruled on this exact bot, and his ruling was: cut the waste, KEEP the monitoring.** A full `Disallow: /` is a larger and different decision than the one he approved, and it would contradict a documented preference of his to buy a benefit I cannot size — **I have no Vercel cost figure, so "reduce load" is speculative, while the loss of brand-monitoring coverage would be real.** Awario contributes nothing to search or AI citation, which is why I had it filed as an easy win. **Not shipping it unilaterally. It goes to NEEDS HENRIK as a one-line question instead, which is cheap. The honest reframing: this was never mine to decide, and I had it filed as if it were.** | **medium — now a QUESTION for Henrik, not a pending change** |
...
**No new experiment today — SEVENTEENTH consecutive day. The reason is the
plainest it has been: both of today's commits are backend instrumentation and
a measured doc correction. `9f420e3` changes what a cron logs about itself;
`b6979ae` changes a comment. Neither is in the sitemap, neither carries
indexable prose, neither can move a ranking or a citation. There is no
hypothesis here to register, and registering one anyway is the retrofitting
this ledger exists to prevent.** **O-88 remains the only candidate with a real
pre-change baseline and it is still Henrik's call, not my experiment.**

**Yesterday's reason, kept because it still stands:**
...
**Next read-outs: 09-13 (`9f420e3` — the three new superseded fields must be
PRESENT in `cron_logs` and sum to `snapshot_superseded`; absent ≠ zero),
09-14 (O-6 / CompareLedgerPulse — needs Henrik's GSC Generative-AI re-export,
see BLOCKED), 09-19 (re-measure the `property_pricing_history` ↔
`price_snapshots` agreement), 09-23 (/statistics hub), 09-25
(sitemap-frontier re-read).** **09-12's `1a1832d` read-out is DONE and closed.**

**Weekly search scan: DONE 2026-09-09, next due 2026-09-16. NOT re-run on
09-12 either, for the same reason.** (Older note, kept:) **NOT re-run today —
...
|---|---|---|---|
| AVM median absolute error | **15.75%** (in-sample, n=**2,033**), MAPE 21.94%, RMSE 43.15%. **My gate run reproduced the committed file EXACTLY — every figure identical, only `computed_at` differs.** **AND IT CORRECTS YESTERDAY'S CORRECTION, which is worth a sentence because I got this wrong in both directions in two days.** Yesterday I "corrected" 15.75 → 15.71 as a transcription error. The truth is that BOTH were right on their own day: the nightly regenerates this file, so 09-10's book gave 15.71 / n=2,036 and 09-11's nightly rewrote it to 15.75 / n=2,033. **The number moves with the BOOK, nightly, and comparing today's gate run against a figure I wrote down two days ago is comparing two different samples.** The only valid comparison is gate-run vs the CURRENTLY committed file, which is what the gate actually does. No AVM code has been touched since 08-07 | 2026-09-12 | `public/model-stats.json` |
...
| **ENVIRONMENT FACT — the session starts in DETACHED HEAD, every single morning (confirmed again 09-12; `git branch -f main HEAD && git checkout main && git fetch origin main` recovered it cleanly, then two pushes went through 1 commit ahead each time)** | `git status -sb` reads `## HEAD (no branch)` and the local `main` ref lags origin (**today it claimed to be AHEAD by 44 — the stale local ref reads as ahead, not behind, which is the more misleading direction**). Pushing without fixing it sends the STALE ref and is rejected as non-fast-forward with a message that reads like a race and is not one. **The fix is `git branch -f main <origin sha> && git checkout main`, NOT a force-push.** **2026-09-11: I committed on the detached HEAD and only then fixed the ref — `git branch -f main HEAD && git checkout main && git fetch && git pull --rebase` recovered it cleanly and the push was 1 commit ahead as expected. Works either order, but ALWAYS `git fetch` before believing any ahead/behind count.** | 2026-09-11 | `git status -sb`, `git branch -v` |
| Live book | **2,036 listings** (09-12 feed, landed 06:24). At 05:50 it had not landed and `pricing-history` correctly reported `skipped / overdue:false`; the 07:05 self check-in confirmed it rather than assuming it | 2026-09-12 07:06 | `feed-meta.json`, `cron_logs` |
| Sitemap | **2,684 `<loc>`**, valid XML, HTTP 200. Down 3 from 2,687, matching the book's 2,036 → 2,033 — the right behaviour. 5 sampled property URLs all 200 (SP0444, N9175, N8748, N9701, plus `/towns/torremolinos-malaga`) | 2026-09-12 | `/sitemap.xml`, parsed |
| Corpus version | **THREE-WAY AGREEMENT among the surfaces I can read: site `v2026-09-11` = local `public/open-data` `v2026-09-11` = avena-data mirror `v2026-09-11`, all with identical `generated_at` 2026-09-11T09:35:11.487Z (fifteenth correct prediction).** Measured 05:50, before today's rebuild — so `v2026-09-11` on 09-12 is the EXPECTED state, not staleness. **HF remains 401 (no token) and is still the ONLY unverified surface** | 2026-09-12 | site + local + avena-data raw |
...
| **`property_pricing_history` — and it AGREES with `price_snapshots`** | **394,000 dead `listed` rows (2026-04-29 → 2026-08-05) + 167 `increased` (from 08-12) + 40 `reduced` (from 08-14).** **CROSS-CHECKED 2026-09-12 (O-87 closed): over 08-10 onward, 273 snapshot-derived moves vs 260 event rows, 260/260 matching on ref AND day, zero orphans; the 13 unmatched are all 08-11, the day before the log started.** `price_snapshots` remains the ground truth for COMPLETENESS — the log starts 08-12 and is written by a second writer on its own schedule — but its CONTENT is now measured, not assumed. Re-measure 09-19 | 2026-09-12 | direct SQL |
...
| **Real price moves by day** | **24+ on 09-11 — CORRECTED, and the correction is the point: I recorded 19 yesterday from the 06:37 run, and the 14:30 run detected 5 MORE (moves_detected 19 → 24) because a third book arrived during the day. The count is therefore SAMPLE-RATE DEPENDENT: more runs see more moves. Quote the `price_snapshots` day-boundary diff (one price per ref per day), never a single run's counter.** 15 (08-14), 4, 1, 0, 15, 10, 10, 18 (08-21), 9, 0, 0, 3, 6, 5, 6 (08-28), 6, 0 (08-30), 1 (08-31), 6 (09-01), 12, 16, 19, 19 (09-05), 1 (09-06), 0 (09-07), **26 (09-09 — the largest single day in the series)**. **Definition: a move counts only when the previous snapshot is ≤2 days earlier** | 2026-09-12 | `price_snapshots`, direct SQL diff |
| Snapshot rows by day | 2,026 (09-05), 2,034 (09-06), 2,034 (09-07), **2,037 (09-08)**, **2,037 (09-09 — union day)**, **2,036 (09-10)**, **2,035 (09-11 — TWO writes, UNION DAY, corrected from yesterday's 2,033; final book 2,033)**, **2,036 (09-12, single write at 06:24:08 so far — union status NOT asserted until 09-13, per O-92)**. **NINETEEN consecutive days captured through 09-11; 09-12 makes TWENTY, no gap.** (Corrected the same day: I first wrote "twenty through 09-11", which was off by one against my own 09-11 row.) | 2026-09-12 | `price_snapshots` |
...
| **TODAY'S CAPTURE (09-12) — LANDED AND COMPLETE. DAY 20.** | **Feed landed 06:24:08 (2,036 listings, up from 2,033) — ahead of the usual O-61 window for once.** Route at 07:06: `snapshotted` **2,036 of a 2,036 feed**, `trusted_prior` true, `prior_date` 09-11, `prior_age_days` 1, `overlap` **0.999**, `errors` null. **16 moves, all 16 `moves_already_logged` — parse-feed wrote them first, O-20 behaving as documented.** **Confirmed in the DB rather than from the route: `price_snapshots` 09-12 = 2,036 rows / 2,036 refs / 1,016 distinct prices, ONE `created_at` instant (06:24:08).** `sold_properties` 146 → **147**, one departure, so the route's `delisted: 0` is meaningless as always. **DELIBERATELY NOT CALLED A CLEAN SINGLE-WRITE DAY: per O-92 that can only be judged on 09-13. Single write SO FAR.** | 2026-09-12 07:06 | `cron_logs`, `price_snapshots`, `sold_properties` |
| **09-11 CAPTURE — COMPLETE, DAY 19, AND A UNION DAY (corrected 09-12).** | Feed landed 06:35:30 (2,033); capture on the first non-skipped attempt at 06:37:01: `snapshotted` 2,033, `trusted_prior` true, prior 09-10 at 1 day, `overlap` 0.997, `errors` null, **19 moves.** **A SECOND feed refresh landed 09:34:41 and parse-feed banked 2 more refs (N8967, N9998) at 09:34:40; the 14:30 run then found 2 DIFFERENT refs gone (N8972, SP0664) and 5 further price moves. Stored 2,035 refs against a final book of 2,033 → union day.** My 06:40 verdict of "single write, clean, quotable" was wrong by three hours (O-92). **The day is CAPTURED and complete — 20 consecutive days — only its listing COUNT is unquotable.** `sold_properties` 139 → 145 that day, so the route's `delisted: 0` was meaningless as ever (O-20) | 2026-09-12 | `cron_logs`, `price_snapshots`, `sold_properties` |
...
| Delistings | **146 tombstones: 32 correct · 101 one day late · 13 stamped behind = 114 wrong (78.1%)** — re-derived 09-12. **Grew 145 → 146 (ONE new departure). The rate FELL for the first time and it is CONTAMINATION, not progress: `correct` rose 30 → 32 purely because 09-11's union write handed two departing refs a same-day snapshot.** Unmet gate under Plan B Release 1, whose fire date (09-07) passed without firing | 2026-09-12 | `sold_properties` × `price_snapshots` |
| Build health | **NO NEW RED. Every GitHub Actions run since 09-10 is `success` — feed-refresh 68/69, capture-backstop 11–14, pulse-alerts, indexnow. The last failure remains run 66 (09-10 06:35, the RedSP interstitial, O-27's fourth instance), already recorded. No red check on any branch; no open PRs.** **No workflow run at all yet on 09-12 at 05:50, which is normal for the O-61 window.** **Two pushes to main today (`9f420e3`, `b6979ae`), all gates green before each.** Reds in `cron_logs` over 27h are ALL known and attributed: prometheus ×4 + generate-briefs + predictions-generate + causal-update + pulse (Anthropic balance), auto-post ×3 (O-53), cassandra (CORRECT — 09-11 was a run day with no measurement), eu-stats-ingest (istat alone), counterpart-discover (O-41), atlas ×3 (Perplexity 401, from Friday 09-11; **09-12 is a Saturday and NOT a run day, so no atlas run today is correct**). **Nothing new or unexplained** | 2026-09-12 | `cron_logs`, `actions_list` |
| Test coverage added by Odyssey | `test-open-dataset` 27 · `test-scribe` 22 · `test-cron-coverage` 99 · `test-integrity` 15 · **`test-capture-integrity` 33 (was 19 — +14 for the superseded split)** · `test-chunked-write` 71 · `test-prometheus-reporting` 25 · `test-capture-backstop` 37 · `test-eu-stats-keys` 40 · `test-resilient-fetch` 46 · `test-corpus-count` 14 · `test-bis-parse` 32 · `test-openapi-stats-sources` 19 · `test-cbs-parse` 35 · `test-agent-status` 48 | 2026-09-12 | `9f420e3` |
...
| **Crawler ledger, 7 days** | **AwarioBot 29,461 hits / 2,276 paths (O-88, 3.0x Googlebot; path count EXACTLY 2,276 for a second day — the O-14 frozen-set pattern)** · Googlebot 9,812 / 4,925 · PetalBot 6,894 / 4,152 · AhrefsBot 1,969 · Amazonbot 1,653 · bingbot 1,435 / 480 · SemrushBot 1,029 · ClaudeBot 512 / 85 · YandexBot 421 · **OAI-SearchBot 353 / 119** · **ChatGPT-User 244 / 36** · Applebot 235 · Bytespider 162 · SERanking 151 · **PerplexityBot 129 / 106** | 2026-09-12 | `crawler_hits` |
...
| **CITATION ENGINE — DARK SINCE 2026-08-31, DAY 13; LAST REAL MEASUREMENT 08-28, FIFTEEN DAYS** | **09-12 is a SATURDAY and NOT a run day (Mon/Wed/Fri), so no atlas run today is correct, not a failure.** Friday 09-11 failed all three attempts on `Perplexity HTTP 401` — 74 queried, 74 failed, 0 measured, 0 rows written. **`/api/citation-agent/status` now publishes this: `status: failing`, `consecutive_failures: 12`, `measurement_age_days: 15` (up from 14 — the derivation is live, verified today).** The `9171dce` guard is holding: no fabricated 0.00% in thirteen days. **Next run day: Mon 09-14, and it will fail identically unless the balance is topped up.** **`plab-run`'s `keys: {perplexity: true}` means the env var is SET, not that it has credit** | 2026-09-12 | `cron_logs`, live endpoint |
...
**Lesson, 2026-09-12 (the most useful thing today, and it is about my own
routine — A VERIFICATION ISSUED BEFORE THE THING IS FINISHED IS A PREDICTION):**
yesterday at 06:40 I wrote that 09-11 was "a SINGLE write, clean, NOT a union
day, its listing count IS quotable", and committed it. By 09:34 a second feed
refresh had banked two more refs; by 14:30 two others had gone. The day ended
as a union of THREE books, 2,035 stored against a final book of 2,033. **The
claim was false within three hours of being written, and nothing about how I
checked was wrong — I checked a day that was not over.** Every morning verdict
I have issued on union status has the same flaw and has been right mostly
because most days have one refresh. **The rule: a property of a UTC day can
only be verified after that day ends. My "quotable" column must lag by one
day, and 09-12's own status is deliberately left pending in this file rather
than asserted.** Filed as O-92. The generalisation beyond capture: **when I
pre-register a verification, I must also pre-register WHEN the evidence is
complete — a read-out date is not just "did it work", it is "is the answer
knowable yet".**

**Lesson, 2026-09-12 (second — ONE NUMBER FOR TWO OPPOSITE CONDITIONS IS THE
RECURRING BUG WITHOUT A ZERO IN IT):** `snapshot_superseded` reported `2` twice
on 09-11 and meant opposite things: at 09:35 two refs banked by the OTHER
writer from a NEWER book (nothing stale but the run itself), at 14:30 two refs
genuinely gone. I built that field, documented it carefully, and read it wrong
for a day, because the value cannot express which condition produced it.
**The family I had been grepping for is "a failure that becomes a zero"; this
is the same defect with no zero anywhere — a value that is arithmetically
correct and semantically ambiguous.** The discriminator existed the whole time
(`created_at` against the book's generation instant) and was simply never
asked for. **Grep target: any reported count whose members could have arrived
by two different mechanisms — and check whether the field can tell you which.**
...
**2026-09-12 — `1a1832d`'s TRACKING half verified, and it is the half that
could have failed.** Prometheus `consecutive_failures` moved **26 → 30** — four
fires in one day, the predicted number rather than merely a different one —
and `measurement_age_days` moved **14 → 15**. Atlas stayed at **12**, which is
the correct behaviour on a Saturday for an agent that runs Mon/Wed/Fri; a
move there would have been the bug. `observed_at` was fresh, `errors: []`.
**A frozen 26 or a frozen 14 was the discriminating failure and neither
occurred, so the status is derived and not a literal wearing a function's
clothes.** One note kept: the first fetch served a 20-hour-old `PRERENDER` and
my request regenerated it — the same ISR caveat recorded for `9053781`, so
always read the SECOND fetch.

**2026-09-12 — O-87's open question closed by measurement (`b6979ae`).**
`property_pricing_history` and `price_snapshots` do not disagree anywhere:
273 snapshot-derived moves vs 260 event rows over 08-10 onward, **260/260
matching on ref AND day, zero orphan events**, and every one of the 13
unmatched moves dated **08-11 — the day before the event log began writing**.
**The conclusion did not change and that is the point: the reason to keep read
paths on `price_snapshots` is COMPLETENESS, not doubt about the log's
content.** An expired fact got replaced by a measured one rather than by a
newer guess.
