...
| 2026-09-05 | **OPERATIONAL, no commit: I did NOT fire the ~05:37 `workflow_dispatch` of feed-refresh, for the first time in ten days.** Replaced with a conditional backstop that checks at ~06:57 UTC and dispatches only if nothing landed | **TOMORROW 09-06, and the criterion is the CAPTURE, not my comfort.** `select count(*), count(distinct ref), min(created_at), max(created_at) from price_snapshots where snapshot_date='2026-09-05'`. **PASS = the day is captured once, by a scheduled run, with a single `created_at`.** **REFUTED = no rows (I traded a certain capture for a lost day — revert to the blind dispatch immediately and say so), or two `created_at` values (the backstop fired too eagerly and I recreated the union I was avoiding).** **Reason for the change is in O-61: on 09-04 my dispatch banked a strictly WORSE book** | **PENDING — 09-06** |
...
| O-61 | **GitHub's scheduler is late rather than absent, DAY 10 — and TODAY I STOPPED DISPATCHING BLINDLY.** Scheduled runs have now landed AND succeeded four days running: 09-02 06:21, 09-03 06:24 + 09:40, 09-04 06:29 + 09:32 | `actions_list`; **the decisive new evidence is the 09-04 book diff below** | **THE ARGUMENT FOR THE DISPATCH INVERTED TODAY.** I diffed 09-04's three `data.json` blobs: my 05:41 dispatch book held **2,033**; GitHub's 06:33 book held **2,037** — a strict SUPERSET (4 added: N9988, SP1860, SP1861, SP1862; **0 dropped, 0 price differences**), and the 09:34 book was byte-identical to it. **So my dispatch did not merely add a redundant draw — it banked a systematically EARLIER, less complete book, and 09-04 was only completed because a second run arrived to fill it in. Had it not, four genuine listings would have been missing from that day of the moat permanently.** **Replaced with a conditional backstop: check at ~06:57 UTC, dispatch only if nothing landed, escalate rather than dispatch eagerly, hard deadline ~09:00 UTC.** Fewer draws, same day-loss protection. **This is still a workaround; `GITHUB_DATA_TOKEN` remains the fix.** **No day lost: 08-27..09-04, all ten captured** | **HIGH — day 10, posture changed** |
...
| Live book | **2,037 listings** (was 2,033 as I recorded it, which was the 05:41 dispatch book, not the day's final) | 2026-09-05 | `public/data.json`, feed commit `26bb7f5` |
...
| Snapshot rows by day | 2,047 (08-28), 2,042, 2,044 (08-30), **2,044 (08-31 — UNION, true final book 2,042, O-74)**, 2,043 (09-01), 2,033 (09-02), 2,034 (09-03), **2,037 (09-04 — two writes, but a clean superset; equals the day's true final book)** | 2026-09-05 | `price_snapshots` |
...
| **Today's capture (09-05)** | **NOT YET CAPTURED at 05:45 UTC — and that is expected, not an alarm.** No feed-refresh run exists for 09-05 yet; the scheduler has been landing ~06:2x. `pricing-history` at 02:20 and 05:39 both correctly logged **`skipped` — "stale feed — deployed book predates today"**, `feed_generated_date: 2026-09-04`, `feed_age_days: 1`, `overdue: false`. **A `skipped` here is the pipeline reporting honestly that there is nothing new yet; it is NOT the zero-snapshot emergency, which would be a skip or zero on a day the feed HAD refreshed.** Backstop armed for ~06:57 | 2026-09-05 | `/api/cron/pricing-history`, `actions_list` |
