...
| **Real price moves by day** | 15 (08-14), 4, 1, 0, 15, 10, 10, 18 (08-21), 9, 0, 0, 3, 6, 5, 6 (08-28), 6, 0 (08-30), 1 (08-31), **6 (09-01)**, **12 (09-02)**, **16 (09-03)**, **19 (09-04)**, **19 (09-05)**, **1 (09-06)**. **09-06 verified by direct SQL diff, not by the route's `moves_detected` field — they agree.** A quiet Sunday: 8 arrivals, 0 departures, 1 reprice. **Definition: a move counts only when the previous snapshot is ≤2 days earlier** | 2026-09-06 | `price_snapshots`, direct SQL diff |
| Snapshot rows by day | 2,047 (08-28), 2,042, 2,044 (08-30), **2,044 (08-31 — UNION, true final book 2,042, O-74)**, 2,043 (09-01), 2,033 (09-02), 2,034 (09-03), **2,037 (09-04 — two writes, clean superset)**, **2,026 (09-05, ONE write)**, **2,034 (09-06, ONE write at 06:27:44)** | 2026-09-06 | `price_snapshots` |
...
| **TODAY'S CAPTURE (09-07) — NOT YET LANDED AT THE TIME OF WRITING, AND THAT IS NORMAL, NOT AN ALARM** | At 06:08 UTC there is **no 09-07 feed run created yet** and **no 09-07 row in `price_snapshots`**. The 05:10 slot has landed 06:15–06:43 for six nights (65–93 min queue lag), so this is INSIDE the normal window. **`pricing-history` confirms the healthy pre-nightly state rather than the emergency: `skipped`, `reason: stale feed — deployed book predates today`, `feed_age_days: 1`, `overdue: false`.** **The emergency shape is a skip or a zero on a day the feed HAD refreshed and deployed — this is not that.** **STANDING RULE APPLIES: any observation of the GitHub scheduler before ~11:00 UTC is worthless as evidence of ABSENCE (corrections 08-28 and 08-31, the same error twice). I have NOT concluded the nightly failed.** **The capture is protected: the backstop's 06:07 dry-run confirmed it currently reads 09-07 as uncaptured and decides `dispatch`, so its 07:00/09:00 slots will re-dispatch the feed if the nightly has not landed by then.** **TOMORROW: confirm 09-07 was captured — `price_snapshots` for 09-07 should hold ~2,03x rows with a SINGLE `created_at`, and if the backstop dispatched, `dispatchFeedRefresh`'s POST will finally have executed** | 2026-09-07 06:08 UTC | `actions_list`, `price_snapshots`, pricing-history route |
