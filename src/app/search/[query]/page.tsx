...
export default async function SearchQueryPage({ params }: { params: Promise<{ query: string }> }) {
  ...
  if (!parsed) {
    return (
      <div className="min-h-screen flex items-center justify-center text-white" style={{ background: 'hsl(var(--av-background))' }}>
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">Search Not Found</h1>
          <Link href="/" className="text-primary">Back to Terminal</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen text-gray-100" style={{ background: 'hsl(var(--av-background))' }}>
        ...
        {results.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              ...
              <div key={s.label} className="rounded-xl p-4 text-center border" style={{ background: '#0f1419', borderColor: 'hsl(var(--av-border))' }}>
        ...
        {top15.length > 0 ? (
          <>
            ...
            <div className="space-y-2">
              {top15.map((p, i) => (
                <Link key={p.ref ?? i} href={`/property/${encodeURIComponent(p.ref ?? '')}`} className="flex items-center gap-4 border rounded-lg p-3 hover:border-primary/30 transition-all" style={{ background: '#0f1419', borderColor: 'hsl(var(--av-border))' }}>
                  <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${i === 0 ? 'bg-primary text-black' : 'bg-[hsl(var(--av-border))] text-white'}`}>{i + 1}</span>
                  ...
                  <div className="text-right flex-shrink-0">
                    <div className="text-primary font-bold text-sm">&euro;{p.pf.toLocaleString()}</div>
          ...
          <div className="text-center py-16 border rounded-xl" style={{ background: '#0f1419', borderColor: 'hsl(var(--av-border))' }}>
            <p className="text-gray-400 mb-2">No properties match this search right now.</p>
            <Link href={`/towns/${parsed.townSlug}`} className="text-primary text-sm hover:underline">Browse all properties in {parsed.town}</Link>
          </div>
        )}

        {/* link to parent town */}
        <div className="mt-8 text-center">
          <Link href={`/towns/${parsed.townSlug}`} className="text-primary text-sm hover:underline">View all properties in {parsed.town} &rarr;</Link>
        </div>
      ...
      <footer className="border-t py-6 text-center text-gray-600 text-xs" style={{ borderColor: 'hsl(var(--av-border))' }}>
